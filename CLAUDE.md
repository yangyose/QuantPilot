...
- **验证覆盖的是不是「实际走的那条路径」**（2026-09-03~09 一周内三次，含上表第 7 行那例）：①用「F-4 跳过警告消失」验回填，而那条警告只在**恰好 100% 全 NULL** 时才响（`1b05bc8` 前，1.2% 与 99% 在它眼里一样）；②设计用 `period=` 定期调用做 Tushare「真调核对」，而回填实际走的是**日期窗口**调用（有 100 行硬上限，§4.3）。都不报错，且都带着一份「已核对」的记录。**核对前先问：生产/回填实际发的是哪一种调用、打的是哪一个字符串？照那个来。**
