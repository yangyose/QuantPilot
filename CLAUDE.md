...
| **V1.0 收尾批次（Phase 11~15）** | **2026-05-14 V1.0 重新定位** — 2026-05-13 5y 真机验收后用 2026-05-13 当日 pipeline 验证发现历史所有 trade_date 的 `signal` 表 0 行（候选池顶分 71.19 < buy_threshold 80），根因是核心评分公式存在三个数学缺陷：① rank-pct 跨期不可比；② 4 策略横截面天然反相关（trend ↔ reversion）锁死 composite 顶分 70~75；③ 绝对阈值 80 与实际评分分布脱钩。V1.0 重新定位为"**所有阻断用户达成核心目标的问题修复完成才能发布**"，原 V1.5 中评分链路 / 因子级溯源 / 数据质量监控 / 部署评审 / 账户资金链项目升级 V1.0 必修，新增 Phase 11~15：**Phase 11 评分工业化 ✓ 完成 2026-05-18 + Phase 12 因子级溯源 ✓ 完成 2026-05-20 + Phase 13 生产可观测 ✓ 完成 2026-05-22**

Phase 11（流派 1 完整版交付：5 步管线 Winsorize / 行业+市值中性化 / Z-score / Gram-Schmidt 正交化 / 三层输出 + ICIR 滚动加权 + Hysteresis + 月末 rebalance + 分位阈值 + 双重失效止损；5y 真机跨制度回归 4 trade_date × 3 state 全 PASS：**composite_z=3.2~4.6**（v1.4 修复后落 N(0,1) top 0.05% 合理）/ composite_score ≥ 99.94 / pct=0.0004~0.0005 / pool_count=50 / BUY signals 43~50 每日 / market_state PIT 100% 一致；**实施期发现并修复 4 处 bug**：(1) P0 PIT bug——`_run_phase11_pipeline` 原用 `get_latest_market_state()` 无 before_date 取全表最大日 → 改 `before_date=trade_date+1d`（集成测试 INT-P11-SC-05 覆盖）；(2) Scorer Robust Z-score 漏标准化——策略内 `z_df.mean(axis=1)` 后未再 standardize，momentum 因子 NaN 率 33% 导致 strategy_z 顶值 11.24 → 加再 standardize + clip ±3.5σ；(3) Orthogonalizer 共线退化阈值 1e-12 太松——momentum 残差被前序投影吸收 99.99% renormalize 把 z=-0.089 放大 270 倍 → 加 collinear_residual_ratio=0.3 阈值（R²>91% 视为共线整列剔除）；(4) pool_capacity 默认 20 截断 STRONG → 升 50；docker COPY src/ 镜像 stale 教训写入 memory；**known issue**：weights_source 全 default_matrix（factor_ic_window_state/strategy_weights_history 仍空，需 5y candidate_pool 历史回填 + ICIR 滚动累积 272 日 → Phase 14 §14-2）/ STRONG count 18~23 vs 设计 30 偏差（universe 过滤后 ~2400 只，top 1% ≈ 24，绝对数基线 30 假设全市场 3200，留 Phase 15 RC 复审）/ BacktestEngine 仍走 aggregate_legacy + 派生 z（不影响实盘 critical path，Phase 14 §14-2 接入真 5 步）；§10.4 "3 state × 10 trade_date = 30 日完整版"拆分 Phase 15 RC，理由：5y 数据上每日 pipeline 130~250s（buffer 热），30 日全跑 ~1.5 小时与 5y 全 pipeline 回填一并处理更合理）/ **Phase 12 因子级溯源 ✓ 完成 2026-05-20**：P12-A LineageService 重构（19 字段 SignalLineageResponse + candidate_pool join；附带修复 Phase 7 起埋藏的 `signal.signal_date` 长期 bug——ORM 列名 `trade_date`，旧代码因 SAPI-05 全 mock 而未暴露）+ P12-B AttributionService OLS（engine/attribution/regression.py 纯函数 + 月末批 run_monthly 写 `attribution_history` + 端点 `GET /attribution/{history,summary}`；**实施期发现并修正设计 §2.2 措辞**：v1.1 写"读 factor_neutralized → 4 strategy_z"，实际 factor_neutralized 是 Step 2 输出 `{strategy: {factor: float}}`，需重做 Step 3 才得 strategy_z；改为直读 score_breakdown_raw[strategy]["z_raw"]，两路径数值等价，v1.2 修订非降级是去歧义）+ P12-C 前端三层折叠（SignalCard L1 trigger_reason 入口 + SignalLineageView L1/L2/L3 + AttributionPanel ECharts 4 因子 β）+ P12-D 单元 6 + 集成 6 + E2E 7 + 冒烟 API-90~95 全 PASS；生产栈已部署 alembic 0010/0011（attribution_history 新建 + candidate_pool 补 3 JSONB 列）/ **Phase 13 生产可观测 ✓ 完成 2026-05-22 + P0/P1 补丁批 2026-05-22 完成**：A 段 MetricsRegistry 7 Counter + 3 Gauge + 2 Histogram + SchedulerHealthService + `/metrics` + `/health/scheduler` + `/health/data` + 4 处埋点（pipeline/signal/notification/scheduler）/ B 段 alembic 0012 DataQualityMetric 表 + Repository + DataService.ingest_daily 写入 + FactorMonitorService.check_persistent_decay（3 月 ICIR < 0.05 触发 factor_decayed_persistent）+ NotificationService.notify_health_alert / C 段 SecretFilter 5 类正则 + AKShareAdapter 补 fetch_daily_quotes/fetch_index_history + DataService Tushare→AKShare fallback + DATA_SOURCE_FALLBACK 4 状态 / D 段 Redis lifespan + DailyPipeline 12 个埋点 + WS /api/v1/pipeline/progress + frontend WebSocketClient + DashboardView PipelineProgressCard / E 段 docker-compose.monitoring.yml profile=monitoring + Prometheus + Grafana provisioning（Overview 7-panel）+ nginx /metrics 内网白名单 + deployment.md §13 / F 段 冒烟 API-96~101 + 收尾。**P0/P1 补丁批（评审 `docs/reviews/phase13_implementation_review_2026-05-22.md` §8）**：R13-P0-1 trigger 注入 redis+notification_channel / R13-P0-2 AKShare fallback 用 PIT 活股 + 1000 上限 / R13-P1-1 PIPELINE_DURATION 6 step + BACKTEST_QUEUE_DEPTH inc/dec + DATA_LATENCY 双路径 set / R13-P1-2 apply_monthly_rebalance 接入 check_persistent_decay + notifier 注入 / R13-P1-3 pipeline 失败改 notify_health_alert("pipeline_failed") / R13-P1-4 vite ws:true + frontend/nginx WS upgrade location。测试 28 UT + 6 E2E + 2 INT（设计） + 11 UT/1 E2E（补丁批）= 552 unit+e2e PASS；回归 integration 118 + ruff 0 error + vue-tsc 0 error。Phase 12 收尾时预存在 API-83 yaml.YAMLError 422 已处置 / **Phase 14 账户资金链 + 5y candidate_pool 回填 + ICIR 历史回算 + BacktestEngine 真 5 步**（含 RM-13 + Phase 11 实施评审 §6.3 推迟两项：ICIR 窗口改严格交易日 + factor_ic_window_state daily/aggregate 共表评估拆分）/ **Phase 15 RC 验收 + 文档同步**；估算 ~38-55 pd（8-11 周）；V1.0 发布后 V1.5+ 路线见 `docs/design/v1_5_roadmap.md`（v2.0 重构后 34 项 ~56-82 pd）| **Phase 11 + 12 + 13 ✓ / 14~15 待启动** |
...
> 此条与 §5 TDD 工作流 / §10 Phase 治理规则并列，是项目所有决策的元规则。

**默认行动：现在的问题现在处理，不推迟。**

V1.0 收尾批次（Phase 11~15）期间用户反复强调的原则，从 [[feedback_fix_bugs_eagerly]] memory 升级为 CLAUDE.md 正文：

- **默认立即修**——评审、TDD、调试、跨 phase 交接中发现的所有 bug / 缺陷 / 设计偏差，**首选当下解决**
- **必须先写下"推迟的充分理由"才允许 defer**——理由必须落到 system_design §9 对应 phase 行（或评审报告修订追踪表）
- **充分理由的判定标准（满足任一即可，缺一不可）**：
  1. **依赖外部决策**——例如需要金融专家锁定参数、需要用户对产品策略拍板
  2. **大重构跨 phase**——单项修复会牵动其他 phase 设计文档（如 Phase 14 §14-2 ICIR 历史回填依赖 5y candidate_pool）
  3. **验收标准未定义**——修了之后无法判定是否修对（如覆盖率门槛 ≥ 90% 需所有 phase 跑完）
  4. **物理资源约束**——例如月末批 Job 必须等月末日期到才能真机验收
- **禁止以下"伪推迟"**：
  - "不影响主路径所以推后"——但实际影响可观测性 / 数据完整性 / 合规视觉一致性
  - "V1.0 范围之外"——但本身就是 V1.0 收尾 phase 的实施评审发现项
  - "Phase X 一起做更高效"——但 Phase X 的设计尚未落地，承诺可能落空
  - "修了也只是小幅改进"——但代码改动量 < 30 分钟、有明确正确性收益
- **冲突时的回溯链**：用户挑战"理由是否充分？"→ 用上述标准逐项核对 → 不通过则当下修
- **历史教训**：V1.0 评审推迟到 V1.5 的项目曾累积到 50+ 项（SDD §16 14 项 + V1.0 评审 P2/P3 25 项 + SDD 外部评审 8 项 + Phase 10 评审 3 项）→ 实际进入 V1.0 时发现核心评分公式缺陷阻断用户达成核心目标 → 重新定位 V1.0 + Phase 11~15 5 个补救 phase，~38-55 pd。**推迟不是节省，是债务利息**

**写入提醒**：每次评审报告 / Phase 收尾核查产出推迟项时，本条规则必须被显式引用 + 充分理由必须写出，否则该推迟无效。

---

## 11.1 推迟项防丢失流程（推迟通过后强制执行）

> 「推迟」≠「记在评审报告就完事」。评审报告归档后没人天天翻，必须在主开发者**下一次必读的文档**里留下前向引用，否则推迟项 = 隐形债务。

**根本原因**：开发者启动 Phase N 时先读 `system_design §9 Phase N 行` 和 `phase N 设计文档`，**不会主动翻历史评审报告**。Phase 13 实施评审 P2/P3 11 项若只勾在评审报告 §8，Phase 14 启动时大概率漏掉。

### 推迟项三链必填（任一缺失即推迟无效）

每个推迟项落地必须同时写到 3 个位置，缺一不可：

**链 A：评审报告 §8 修订追踪表**（已经默认会写）
- 含编号 / 等级 / 处置 / 责任 / 截止 / 状态 6 列
- 表末追加 **§8.2「推迟项前向引用位置（防丢失）」** 小节，明示每条 pending 项在链 B/C 的精确位置

**链 B：system_design.md §9 目标 phase 行**（Phase 14/15 等近期 phase）
- 在目标 phase 行的现有 scope 列表里**追加子项**：`(N) **<原 phase> 实施评审 <等级> X 项**（详见 <评审报告路径> §8）——<子项 1>/<子项 2>/.../<子项 X>`
- 子项必须**展开列出**编号 + 一句话描述（不要只写"详见评审报告"——读者必须能直接看到要做什么）
- phase 行末标注「待创建，启动核查时必须列入 N 项 scope」提醒 phase 设计文档作者

**链 C：v1_5_roadmap.md（远期 V1.5+ 推迟）**
- §6 主题打包表对应主题行 scope 列追加 `+ §X.Y <原 phase> 评审 Z 项`，并相应调大 pd 估算
- 新建 §X.Y 小节展开 5 列：编号 / 描述 / 估算 / 所属主题 / 评审报告依据
- 主题标题如需扩展（如 V1.5-A「回测引擎深化」→「回测引擎深化 + 监控增强」），同步修订

### 触发条件矩阵

| 推迟去向 | 必填链 | 触发示例 |
|---------|-------|---------|
| 下一个 phase（Phase N+1）启动核查 | A + B | Phase 13 评审 P2 → Phase 14 §9 行 |
| V1.0 收尾后任一 V1.5+ 主题 | A + C | Phase 13 评审 P3 → roadmap §4.5 + §6 V1.5-A |
| V2.0+ 永久延期 | A + C（标 V2.0）| SDD-EXT-06f 边际 VaR |
| 当前 phase 内补丁批（如 R12-P2、R13-P0/P1）| A 内勾选即可 | 本 phase 自己处理，不算推迟 |

### Phase 设计文档作者的对应义务

启动新 phase 写设计文档时，**§1.3 启动核查清单必须含**：

```
- [ ] grep system_design.md §9 本 phase 行所有子项（含 R<N>-P<X>- 评审追溯）
- [ ] grep `R\d+-P[2-3]-\d+` 跨 system_design + roadmap + reviews/ 三处，确认本 phase 应消费的推迟项全部进入 scope
- [ ] 漏列即违反 §10 文档治理规则
```

未做这步直接开始 phase 实施，等同于让上一轮推迟决策失效——属 §11「伪推迟」判例升级版。

### 历史教训：Phase 13 → Phase 14 推迟流程修补

- 2026-05-22 Phase 13 实施评审产出 6 P2 + 5 P3，初版只写到评审报告 §8
- 用户当日质询「剩余项在文档里有说明后面一定不会忘记吗？」
- 核查发现 system_design §9 Phase 14 行未列 R13-P2 / roadmap V1.5-A 未列 R13-P3 → 推迟项可能在 1~3 个月后被遗忘
- 补丁链路（commit 5e94d44）：三链贯通 + §8.2 防丢失小节 + 本 §11.1 沉淀

### 不要把链 A 当作"备份"

链 A 只是历史日志，不会被任何人主动读。**链 B/C 才是防丢失的真正机制**——主开发者下次写 phase 设计文档 / 启动 V1.5-A 时必读的入口。链 A 内容若不投影到链 B/C，等同于把推迟项扔进黑洞。
