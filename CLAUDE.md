...
- **C4 资金动向代码已交付（2026-09-16，未部署）**：`money_flow` 表（alembic 0030）+ 两表 `money_flow_score`（0031）+ adapter / repo / `ingest_daily` 第 5 段 / `strategies/money_flow.py`（影子权重 0）/ `scripts/backfill_money_flow.py`。5434 已回填 100 日样本（212 B/行 → 2y ≈ 0.57 GB，2 年窗口维持）并用真实管线重评分验证落表 50/50。**2y 回填（先 5434 后生产，生产需 C-1）与部署未做**。⚠️ 探针顺带照出两个 C3 也中招的既有缺陷（策略组装点实为 **6** 处、两个 upsert 的 `ON CONFLICT SET` 手写白名单），均已修在源头并各加一条**不数处数**的护栏，见设计文档 §6.7。⚠️ **C4 转正前置（2026-09-23 钉死）**：回测 bundle **没有** `money_flow`，该策略在回测里逐日全 NaN 被跳过（影子期权重 0 故无影响）；权重一旦 > 0，回测就会静默按「少一个策略」的口径算——`tests/unit/test_backtest_feeds_weighted_strategies.py` 在默认权重矩阵上拦住这一步（把权重改 0.05 即红，已变异验证），补完 bundle 再从 `_UNFED` 删除该项
