...
- upsert：`insert(...).on_conflict_do_update()`；`updated_at` 显式写 `func.now()`。⚠️ **`set_=` 子句是第二个白名单**（2026-09-16 C4 探针照出）：`upsert_candidate_pool_bulk` / `upsert_signal_snapshots` 手写四个分数列，新策略列**只在首次 INSERT 时写入**，重跑同一天（`--force` 回填 / 管线 resume）静默保留旧值 NULL——生产每天都是新行所以从未露馅。策略分数列一律由 `SCORE_COLUMN_MAP` 派生，`test_strategy_score_reaches_db.py` 真编译语句钉死
...
- **「落在入参窗口内」≠「当时可得」——回填与实时跑同一段代码，PIT 语义可能完全相反**（2026-09-07，代价最大的一类）：`fetch_financial_data` 写 `publish_date=交易日`（语义「该日已知」），而它调的 `fina_indicator(period=最近季度末)` 返回该期**最终值**、与 as_of 无关且原先 `fields` 不含 `ann_date`。实时跑时该期未披露、返空 → 正确；**回填历史时早已披露 → 把后来才公布的财报写进当时的行**。存量污染 318 万行，使 universe 本身是用「尚未公布的财报」筛的 → **全部历史回测与面板的绝对水平不可信**。生产每天那一次始终是对的，所以四个月无人发现。**判据：问「这条数据在 as_of 当天真的存在吗」，而不是「report_period 有没有落在窗口内」**——后者恒为真。凡按 period/as_of 取数的接口，必须用**公告日**截断（缺公告日则 fail-closed）。**源头已修 `5f7d57a`（随 `85438a8` 上生产）；存量 318 万行已用 `scripts/repair_financial_lookahead.py` 在 5434 与生产库两边执行完毕**（各 `updated=3181745 remaining=0`，四项独立复核未采信脚本自己的报数）。持续检测在 `scripts/audit_data_integrity.py::lookahead_violations`——源头拦「产生」，它拦「存在」（换人跑旧脚本 / 恢复旧备份 / 手工 SQL 都能让污染悄悄回来，而代码是对的、测试是绿的）。 详见 `docs/reviews/financial_data_lookahead_2026-09-07.md`
- **返回行数「恰好等于一个整数」= 被静默截断的指纹**（2026-09-09 加守卫，已用真实调用复现两次事故）：`fina_indicator` 按**日期窗口**调用有 **100 行硬上限**（5 码跨 5.7 年恰好返回 100 行、每码 18~21 期被截断）→ C2 回填 50 码/批时每股只拿到 2 期，而脚本报告 `ok=5515 fail=0`。同一指纹的还有 `suspend_d` 那次（恰好 **5000** 行，害了 4 个月）。⚠️ **下一条判据（日期是否落在窗口内）拦不住这一类**——返回的日期确实落在窗口内，只是少了一大半。守卫在 `TushareAdapter._call` 统一做（`_warn_if_row_capped`，命中已知上限即 WARNING；普通行数必须静默，否则刷屏即等于没有告警）。~~修法是换调用形态（改 `period=` 定期调用）~~ **2026-09-17 订正：`period=` 形态同样被 100 行截断**——生产 C2 回填首期就报 302 次告警，真调 80 码一批恰好 100 行、只剩 55 个 ts_code（25 码整只丢失）；50 码 × 每码 2 行（update_flag 0/1）恰好 100 就是边界，**每日管线的 `fetch_financial_data` 走的是同一形态**，此前每天都在悄悄丢码。修在 `_fina_indicator_batch`：命中上限即对半拆批重取（TD-13/14 用「>100 行只返前 100」的替身钉死，变异关掉拆批即红）。告警本身立了功——没有它这次照样 `ok=4902 fail=0` 过关
...
- **pandas_ta 传错参数名不报错**：0.4.x 把 `bbands(std=)` 拆成了 `lower_std=` / `upper_std=`，旧名落进 `**kwargs` 被**静默忽略**——列名与数值完全不变，无告警。本仓 `mean_reversion.py` 的 `std=2.0` 因此长期无效（只因默认值恰好也是 2.0 才没出事）。判据不是"读文档确认签名"，而是**写一条「改参数 → 结果必须变」的测试**：接了配置却毫无作用的代码能跑过任何只验证"不抛异常"的测试。**2026-08-27 专项排查证实这不是孤例**：`config_defaults` 有 **12 个字段零引用**（`FactorMonitorConfig` 9 项被 `factor_monitor_service` 顶部同值模块级常量架空、`TrendStrategyConfig.macd_*` 被 `trend.py` 写死的字面量架空、`ScoringPipelineConfig.hysteresis_enabled` 无人读取），且三者**都对用户可编辑** → 改了存库、界面显示已保存、代码永不读取。清单与方法学结论见 `docs/reviews/silent_ignore_audit_2026-08-27.md`，已登记 roadmap §6 V1.5-F（F-SI）。**按名字检索的审计会因同名标识符产生假阴性**：`macd_signal` 撞上同名因子权重键而漏检，写死的 MACD 参数实为 3 个
...
- **e2e 目录任何触达真实 DB 的路径一律以 `E2ETouchedDatabase` 失败**（2026-09-17，`tests/e2e/conftest.py` 给全局 engine 挂 `do_connect` 监听器）：BT-09c 首版漏 mock 日历，本机 5432 恰好开着就绿、CI 无 DB 就报上面那条「different loop」——**同一根因在两个环境长两张脸**，三个 commit 的 CI 连红才追到。e2e 的「无 DB」此前靠 `client` fixture 逐个 stub 查库依赖，漏一个就靠环境碰运气；现在漏了就在连接这一层同一句话失败并指名该 mock 谁。判据：`tests/e2e/test_e2e_forbids_database.py`，以及**别给测试机起 DB 去让 e2e 变绿**。⚠️ 该护栏首版写成 `scope="session"`，又让 CI 连红 4 次：**CI 把 `unit e2e integration` 放在一个 pytest 会话里跑**，session 级 fixture 由 e2e 触发、却活到进程结束，之后走全局 engine 的集成测试全部被拦；本机三个目录分开跑永远看不见。已改 function scope。**推送前至少把 `tests/e2e tests/integration` 放同一次 pytest 里跑一遍**——分目录跑不等于 CI
...
- **同一测试 DB 严禁并发两个集成 pytest 会话**：conftest 的 session 级 alembic（建表 / downgrade base）会互相拆台，典型症状是**单个**测试随机 `UndefinedTableError` 而前后测试全过（表被另一会话瞬时 DROP）。⚠️ **「另一个会话」不限于 pytest**（2026-09-23 踩到）：我在后台 CI 形态会话跑着的时候，手工对同一个 5433 跑 `alembic downgrade -1 / upgrade head` 验证新迁移 → 那次 **36 failed / 434 passed**，报错五花八门（连接层 / 表不存在 / 约束不存在），看起来像代码回归，实则是我自己把表从它脚下抽走了。判据：**跑集成会话期间，对同一个库的任何 DDL（含手工 alembic、`psql` 建删表、`docker compose down` 带卷）都算并发会话**；要验迁移就写成用例（`test_int_plugin_migration.py` 那样在会话内串行跑），**不要在另一个集成会话跑着时另开终端手工跑**（空闲时手工 `alembic upgrade head` 照常，本条限定的是「有会话在跑」这个时段）
...
**且常规测试全绿**。载体各不相同而根因一致——参数 / 配置 / 外部接口（两种形态：返空 / 返错数）/
调用点 / 测试替身 / 测试输入 / **字段名词表**（生产方与消费方不同名）**八种载体均已中招**
（测试替身与测试输入同属"测试侧"但判据不同，故分列）：
...
| 7 | **`SecretFilter` 宣称覆盖 REDIS_URL，生产 Redis 密码明文 4 个月**（2026-09-03）| **测试输入比现实更配合**（与第 5 行的「替身缺状态」同族、载体不同）：`_SECRET_PATTERNS` 匹配字面量键名 `REDIS_URL=`，而 `main.py:72` 实际打的是 `redis_connected url=redis://:<pw>@...`——键名是 `url`。单测 UT-P13-C-01 自造了一个含 `REDIS_URL=` 的串来喂，于是全绿而缺陷仍在。**按键名匹配的护栏只挡得住恰好用了那个键名的调用点**；修法是按 URL **形状**（`scheme://user:pass@`）匹配，与日志措辞解耦 |
| 8 | **滑点对比的「累计收益」列恒 0.00%**（2026-09-23）| **字段名词表分叉，取值方按不存在的那个键取**：对外契约是 `total_return`（前端列 / CSV 表头 / `SlippageScenario`），而 `BacktestReport.generate` 产出 `cumulative_return`；`run_slippage_comparison` 写 `perf.get("total_return", 0.0)` → 默认值 0.0 直接落到界面。**替身与被测代码一起错**（`_StubEngine` 也返回 `total_return`）故单测全绿，DoD 只看 sharpe 单调性故验收也过；同一漂移还让 `run_backtest_local.py` 的汇总静默少打三行。判据：`perf.get("X")` 的每个 X 必须出现在**真实** `generate()` 的键集合里（AST 取字面量 + 真实函数取键集，`test_slippage_report_keys.py`），替身的键另用 `_assert_real_keys` 现场对账。**与第 5/7 行的边界**：测试没查出来不是因为替身缺状态、也不是输入太配合，而是**替身原样复刻了生产代码的错键名** |
...
- **验证覆盖的是不是「实际走的那条路径」**（2026-09-03~09 一周内三次，含上表第 7 行那例）：①用「F-4 跳过警告消失」验回填，而那条警告只在**恰好 100% 全 NULL** 时才响（`1b05bc8` 前，1.2% 与 99% 在它眼里一样）；②设计用 `period=` 定期调用做 Tushare「真调核对」，而回填实际走的是**日期窗口**调用（有 100 行硬上限，§4.3）。都不报错，且都带着一份「已核对」的记录。③（2026-09-16）`liquidity_note` 的 AST 钉子钉在 `strategy_service` 的
  `get_avg_amount` 调用点——那是 universe F-7 的取数路径，喂给 `SignalGenerator` 的快照（`get_snapshot_quotes`）
  **根本没有 `avg_amount` 列**，于是生产 104 条 BUY 文案全 NULL 而钉子一直绿；单测则自带了那一列。
  **核对前先问：生产/回填实际发的是哪一种调用、打的是哪一个字符串？照那个来。** 钉调用点时还要问：
  **这个调用点的产物真的流到被测对象手里吗？**

- **参数是否真被消费** → 写「改参数 → 结果必须变」（§4.4 pandas_ta 那条）
- **配置是否真被读** → 全仓 grep 该字段名，零引用即可疑（注意**同名标识符会造成假阴性**——
  实例见 §4.4 pandas_ta 那条末尾）
- **字段名词表是否对齐** → 生产方（如 `BacktestReport.generate`）与消费方（`perf.get(...)` / 前端列 `dataIndex` / CSV 表头）的**键集合逐一核对**，用真实函数取键、AST 取调用点字面量。⚠️ **`.get(key, default)` 里的 default 本身就是可疑信号**——写得出默认值，说明作者不确定该键在不在；键不在时它就是个假数据，而不是报错
...
- **测试替身是否比现实更能干** → 替身缺了真实对象的某个**状态**，那条分支在测试世界就
  不可表达，于是永远没人写它（上表第 5 例）。判据是**在实例上**逐项比对替身与真实类的
  契约（`uid`/`configured` 这类 property 在类层面 `hasattr` 对替身恒 False，只能查实例），
  并单独钉一条「真实对象在生产那种输入下确实进入该状态」。

- **测试输入是否比现实更配合** → 上一条的姊妹形态，载体是**输入**而非替身（上表第 7 例）。
  给护栏喂一个自己编的、恰好能命中的样本，护栏就永远绿，而现实中那条真实输入从未被试过。
  判据是**用被测代码在生产逐字打出/传出的那个串**（从日志、DB 或调用点抄下来），
  不是照着规则反推一个样本。⚠️ 断言也要跟着换：断言「出现了 `***REDACTED***`」
  在「整行压根没匹配上」时同样可能为真，**要断言的是密码子串不出现**。

**元判据**：一个机制若「设计上应当生效」，就去查**它生效时会留下的痕迹**（数据库里的标记列、
日志计数、非空率）。查不到痕迹 = 没生效，不管代码看起来多对。

⚠️ **痕迹判据本身也要先证伪**（2026-09-01 踩到）：§4.4 那条「改参数 → 结果必须变」不只适用于
测试，同样适用于**可观测性判据**。当时用「`score_breakdown_raw->'momentum'->>'z_raw'` 非空率」
去验证价格窗口修复——但 momentum 权重为 0 时该键**根本不存在**，查询在「修好了」和「没修」
两种情况下都返回 0。**一个判据若在机制生效与失效时给出相同结果，它就不是判据**。
定判据时先回答：它在两种情况下真的会不同吗？答不上来就别用它下结论。
...
钉了必漂：这里曾写死 `28/28`，而规则几轮收窄后早已变了几次；当前数字只在 §5.3 记一处）。**别用手敲的 `echo '{...}' | python guard.py` 自检**：`guard.py` 在 JSON 解析失败时**同样静默 `sys.exit(0)`**,而该写法是 Bash 语法、在 cmd.exe 里单引号不是定界符 → JSON 变脏 → 静默放行,与"守卫已死"表现完全相同(2026-08-26 误判过一轮)。改 `guard.py` 规则时必须往夹具补用例,且**正反两面都钉**——只钉"该拦的拦住",规则写宽了没人发现
...
判据是跑 `python .claude/hooks/test_guard.py` → **59/59 passed**（2026-09-16 起含 `PowerShell` 工具：
此前守卫只看 `Bash`，PowerShell 完全不在视野内——09-14 我自己就用它绕过挂死的钩子、连带绕过全部保护；
现语法无关规则同样适用，写文件动词加了 `Set-Content`/`Out-File`/`WriteAllText`，夹具另钉「settings.json
matcher 含全部被测工具」并做过变异验证）。
...
**进行中：V1.5-C 策略扩展**（设计文档 `docs/design/phases/v1_5_c_strategy_expansion.md`——**不标版本号**，标了必漂；
刚删掉的写死 sha 是同一种失败模式，
...
- **C1 策略约束与风险调整动量**：C1-1（约束落点统一，`ac069e5`）/ C1-2（风险调整动量，
  ...
  约 3 倍）见设计文档 §3.3
- **P0 退出域缺陷**（2026-08-28 发现，修复 `e798e7a` **2026-08-31 已上生产**）：卖出评估取的是候选池而非
...
- **C2 六块全完成 + 5y 回填已在本地算力中心激活**（7 列覆盖 94.7~99.9%、9/9 项可判；Altman Z-Score 备选已裁定放弃；**生产 7 列已于 2026-09-17 `e86daac` 回填，联合覆盖 89.3%、管线实测 `judged=3111 unjudgeable=98`**）/ **C3 低波动策略代码已交付**（51 条单测、影子模式 0 权重、alembic 0029、新建 `core/strategy_registry.py` 单一事实来源）/ **C4 已上生产**（2026-09-16 `57bc966`，alembic 至 0031；2y 回填两边已完成并审计，⚠️ 历史覆盖缺北交所，见设计文档 v0.21）/ **C5 进行中**（2026-09-23：沙箱执行器 + `PluginStrategy` 适配器 + 执行开关（生产默认关）+ §7.4 两表 alembic **0032**；余五端点 + 冒烟 + 专项 security-review + 收尾三项（生产 `printenv` 实证 / SDD §15.2 回写 / `deployment.md` 补「插件执行仅本地算力中心」红线）。⚠️ 沙箱**挡不住蓄意逃逸**，故生产只开「编写/上传/管理/审计」、执行走本地算力中心——设计 §7.1 的诚实声明，不是遗漏）。**C2/C3 已于 2026-09-09 上生产**（`af94e57`，alembic 至 **0029**；核验见 `docs/ops/deploy_log.md`）。⚠️ **该批选股行为应为零变化**——`low_volatility` 影子权重 0、`piotroski_gate_enabled=False`（照常算、照常记日志，不剔除）；观察期看到 universe/signal_count **跳变才是异常**，不是新功能生效。门控经开发集实测**在任何阈值上都无显著收益**，仍为影子模式，待 holdout + 生产影子期独立复现同向改善后再议激活（7 列回填已于 09-17 完成，见本 bullet 句首；那次部署的历史细节在 `docs/ops/deploy_log.md`）
- **C4 资金动向 ✅ 已上生产（2026-09-16 `57bc966`，alembic 至 0031）**：`money_flow` 表 + 两表 `money_flow_score` + adapter / repo / `ingest_daily` 第 5 段 / `strategies/money_flow.py`（影子权重 0）/ `scripts/backfill_money_flow.py`。**2y 回填两边均已完成，2026-09-23 审计**：5434 484 交易日、生产 489 交易日，各约 540 MB、**对交易日历零缺日**。⚠️ **历史覆盖缺北交所**：历史日对当日有行情股的覆盖率约 95%，缺口全是 BJ（Tushare 的 BJ 资金流实质自 2026-08 起全量，此前每天仅 1 只），而 BJ 有 310/345 只过 F-7 阈值、候选池里占 10/68 → **C4 的历史 IC 与回测在 BJ 子集上无观测**，转正评估不得当成全市场结论（设计文档 v0.21）。⚠️ 探针顺带照出两个 C3 也中招的既有缺陷（策略组装点实为 **6** 处、两个 upsert 的 `ON CONFLICT SET` 手写白名单），均已修在源头并各加一条**不数处数**的护栏，见设计文档 §6.7。✅ **转正前置已单独交付（2026-09-23，与上面「C4 未部署」是两条时间线）**：回测 bundle 此前无 `money_flow` → 该策略在回测里逐日全 NaN 被跳过（影子权重 0 故无影响，权重一旦 > 0 就静默少算一个策略）；现已喂上，机制与实测见设计文档 §6.7。⚠️ **切片上下界都不能省**：无上界 = 前视（策略靠 `tail(N)` 取最近 N 行），无下界 = 长期停牌股在生产是 NaN 而回测会拿到窗口外的老行。判据 `test_backtest_money_flow.py` + `test_backtest_feeds_weighted_strategies.py`（后者是「默认权重矩阵里回测喂不起的策略必须 0 权重」的闸门，与 `_UNFED` 登记双向绑定）
...
> **运维红线（RC 验收期实证）**：① 生产机**禁止一切「全 universe 评分」作业**（17:30 每日管线那一次除外）——判据是代码路径是否调用 `score_universe_for_date` / `ScoringService.score_universe`，**不是功能叫什么名字**。已实证会打挂生产的两例：回测（单个 6 日任务拖垮 11 分钟 → `POST /backtest/run` 已 `backtest_enabled=false` 返 503）、日级 IC 回填（`scripts/backfill_daily_ic.py` **仅跑一个交易日** 即 RSS 1.58G 触发 OOM killer，2026-08-17 致站点 530 共 43 分钟）。⚠️ **生产机 2026-09-03 已由 2C2G 升配至 2C4G**（轻量套餐升级，**不可降级**），上面两例的实测数字都出自 2GB 时期——**升配不解除本条**。⚠️ **但下面三条理由到 2026-09-14 已全部过期**——
**本条红线当前没有有效依据支撑，需重新论证**，详见
`docs/reviews/memory_premise_after_4gb_2026-09-14.md`（(a)(c) 是 2GB 时期数字；
...
回测引擎经 PE/PB 下推后 6~30 日峰值 1.0~1.2 GB，见 `core/config.py` 与该报告 §2②；**2026-09-21 分位改为紧凑数组在内存里算**（24 B/行 ≈ 150 MB，语义与 SQL 逐码相同）后 6 / 30 日峰值工作集 1.17 / 1.25 GB、耗时 58 s / 177 s——「历史进内存 = 3.5 GB」的旧结论只对 Row/宽表形态成立）。
