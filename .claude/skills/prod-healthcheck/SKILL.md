...
```bash
docker exec quantpilot-backend-1 sh -c 'for p in $(ls /proc | grep -E "^[0-9]+$"); do c=$(tr "\0" " " < /proc/$p/cmdline 2>/dev/null); case "$c" in *backfill_candidate_pool.py*) echo "RUNNING PID=$p";; esac; done'
```
没输出 = 进程已死 → 进第 4 节做 OOM 诊断。
