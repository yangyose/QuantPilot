...
| P1-2 | P1 | §5 重写采方案 A：v1.1 路径 = engine 层新增 `engine/scoring/pipeline.py::run_scoring_pipeline` 纯函数 + `Scorer.aggregate_pipeline()` 公开方法 + BacktestService 预查 5y active_weights/industry/market_cap 入 BacktestDataBundle | 设计文档 v1.1 | ✅ fixed @ v1.1，**v1.2 短复审收口**（详见 `docs/reviews/phase14_design_review_v1_1_short_2026-05-25.md`）：v1.1 §5.2 设计 "新增 engine/scoring/pipeline.py + Scorer.aggregate_pipeline + InsufficientUniverseError" 是 3 项已存在轮子的误判——既有 `engine/factor_pipeline.py::FactorPipeline` + `engine/orthogonalizer.py` + `engine/scorer.py::Scorer.aggregate` 已完整覆盖 5 步管线 engine 层纯函数；v1.2 §5.2 简化为 "BacktestEngine 直接复用既有 `Scorer.aggregate`" + BacktestService 补加载 `float_mkt_cap` + `active_weights_history` + BacktestEngine 主循环 3 处改造（MarketSnapshot 补字段 / s.score → compute_strategy_factors / aggregate_legacy 与 aggregate 分支二选）+ `WINSORIZE_MIN_SAMPLES=30` 常量新定义于 `engine/scorer.py` + `_lookup_active_weights` helper。§14-3 估算 1-1.5 pd → 0.6-1 pd |
