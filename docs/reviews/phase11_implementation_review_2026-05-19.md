...
| 11 | **P2-4** | `rolling_icir_state` 窗口改用 `TradingCalendar.get_prev_trade_date` 严格交易日 | Phase 14 §14-5 | ✅ **已收口 2026-05-26**（FactorMonitorService 构造器加 `calendar: TradingCalendar \| None`；`rolling_icir_state` 注入 calendar 时 `end = calendar.get_prev_trade_date(t, n=20)` + `start = calendar.get_prev_trade_date(end, n=252)`；4 处生产路径 deps.py / monthly_scheduler.py×2 / daily_pipeline.py 全部注入 `self._calendar`；UT-P14-5-01 + INT-P14-5-01 PASS；SDD §7.4 v1.4 措辞早已写"交易日"无需改）| `services/factor_monitor_service.py` |
