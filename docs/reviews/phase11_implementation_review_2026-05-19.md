...
| 7 | **P2-1** | `scorer.aggregate` 全 NaN 策略跳过路径加 `logger.info` | Phase 12 实施期穿插 | ⏳ 待办（与 P12-A LineageService 改造同 commit）| `engine/scorer.py:154-158, 171-174` |
| 8 | **P2-2** | `SignalResponse.weights_source` 字段冗余 → 删除字段（**前端 grep 无消费已确认 2026-05-19**，可安全删）| Phase 12 实施期穿插 | ⏳ 待办（与 P12-A schema 改造同 commit）| `schemas/signals.py:29` |
| 9 | **P2-3** | `factor_pipeline.neutralize_industry=False` 分支决策（删除 vs 补单测 + 注释）| Phase 12 实施期穿插 | ⏳ 待办（建议保留 + 补单测 + 注释"V1.0 锁定，仅研究模式用"）| `engine/factor_pipeline.py:94-97` + `core/config_defaults.py::ScoringPipelineConfig.neutralize_industry` |
| 10 | **P2-6** | `_DEFAULT_ORDER` 改为按 default_matrix 当前 state 权重降序 | Phase 12 实施期穿插 | ⏳ 待办 | `services/strategy_service.py:33, 439-449` |
...
跨 session 接手 Phase 12/14 时：
1. **先看本表第 7-10 行**：P12-A 实施时同 commit 收口 P2-1/P2-2/P2-3/P2-6（4 项均在 strategy_service.py / signal_service.py / scorer.py / factor_pipeline.py 范围内）
2. **Phase 14 启动时看本表第 11-12 行**：P2-4 与 ICIR 历史回算同 commit；P2-5 与 5y candidate_pool 回填同 commit
3. 每完成一项，把处置状态从 `⏳ 待办` 改为 `✅ 已收口 YYYY-MM-DD`，并在 memory `v1_finalize_deferred_items.md` 同步
