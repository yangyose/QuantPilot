...
> **版本：** v1.0
> **所属阶段：** Phase 3 / 10
> **依据文档：** system_design_v1.1.md §2.6、§3、§5.3；SDD §6、附录 B
> **日期：** 2026-03-27
...
| `main.py` | lifespan 中初始化 `MarketStateEngine`（单例） |
| `pipeline/scheduler.py` | `run_daily_job()` 中 ingest 完成后追加调用 `MarketStateService.identify_and_save()` |
...
class MarketStateEngine:
    ...
    def compute_indicators(self, ohlcv: pd.DataFrame) -> pd.DataFrame:
        """
        输入：ohlcv — columns=[open, high, low, close]，index=date（升序）
    ...
    def identify(
        ...
        prev_confirmed：从 DB 取到的最近一次已确认状态（首次运行传 OSCILLATION）。
        """
...
async def get_latest_market_state(self) -> MarketStateHistory | None:
    """SELECT * FROM market_state_history ORDER BY trade_date DESC LIMIT 1"""
...
> **ORM 类名：** `MarketStateHistory`（`models/market.py` 中已定义）。
...
class MarketStateService:
    ...
    async def identify_and_save(self, trade_date: date) -> MarketStateRecord | None:
        ...
        2. 从 market_state_history 取最近已确认状态作为 prev_confirmed（无记录则 OSCILLATION）
...
**从 `index_history` 读取的字段：** `open, high, low, close`（用于 ADX 计算），以 `trade_date` 为 index 升序排列。
...
依赖注入：`MarketStateService` 通过 `Depends(get_market_state_service)` 注入（`get_market_state_service` 在 `api/deps.py` 中注册，从 `request.app.state` 取出单例 `MarketStateService`）。
...
| MSTS-03 | 状态切换时 `state_changed=True` | 插入前一日 OSCILLATION 状态，计算后切换到 UPTREND → DB 中 `state_changed=True` |
...
| T-09 | `api/v1/market.py` + 更新 `api/v1/__init__.py` + `main.py`（注册路由，初始化 MarketStateService 单例至 `app.state`） | Market API 实现（GREEN → MAPI-01~04） | T-05, T-07, T-08 |
| T-10 | `pipeline/scheduler.py`（在 ingest 后追加 `identify_and_save(trade_date)` 调用） | 调度器集成；`MarketStateService` 与 `DataService` 共用同一 `db_session`（通过 lifespan 注入） | T-07 |
...
- [ ] `uv run pytest tests/ -v` 全部测试通过（含 Phase 1+2 原有测试无退化）
...
**SDD §7.5**（权威来源）与 **system_design_v1.1 §8.1** 的策略权重存在不一致，Phase 4 以 SDD 为准：
...
> system_design_v1.1 中 UPTREND 趋势权重为 70%（与 SDD 40% 不符），DOWNTREND 动量/均值回归权重也与 SDD 相反。**Phase 4 Scorer 实现时统一使用上表（SDD §7.5）。**
