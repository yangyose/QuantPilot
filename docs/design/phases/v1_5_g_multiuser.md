...
> - v1_5_roadmap §2.2「多账户 + 权限粒度（3 项 P2）→ V1.5-G」+ §6 主题表 V1.5-G 行
...
**【设计待定：每日管线的持仓保护】** 每日批 `DailyPipeline` 是全局单次跑（无登录用户上下文）。持仓保护（candidate_pool 对已持仓股的保护、止损预警 Job）原依赖全局唯一账户。多用户后，全局管线产出共享候选池/信号**不含**任何用户持仓视图；持仓保护与止损预警须改为**按用户**触发（登录时实时计算 / 或月末批遍历所有 active user 的账户）。本 phase 设计**先把全局管线与账户解耦**（管线不再读 account），持仓保护移到 **API 请求期按 `get_current_account_id` 实时叠加**；止损预警 Job 的多用户化（遍历 active users）作为 **G-4 子项的明确边界项**详见 §6.4，若工作量超阈则按"跨 phase 大重构"推迟并落三链。
...
### 3.3 数据迁移（alembic 0017，env admin → 首用户）
...
两选一：(a) 注册成功即签发 token 自动登录（体验顺滑）；(b) 注册成功跳登录页手动登录（更稳，便于未来插入邮箱验证关卡）。倾向 **(b)**——为 V1.5-F 邮箱验证留位（验证前不应自动登录）。最终在实施启动核查时锁定。
...
- **止损预警 Job 多用户化**（承 §2 待定）：原止损预警 Job 依赖全局唯一账户。多用户后需遍历所有 `is_active` 用户的账户分别算回撤/止损并各自站内信。**【设计待定：实现还是推迟】** 若改造量在本 phase 阈值内则纳入；若牵动 APScheduler Job 结构 + 通知收件人模型大改，则按「跨 phase 大重构」推迟至 V1.5-F（通知主题），并落三链。实施启动核查时定。
...
- [ ] `user` 表 + `account.user_id` + alembic 0017 迁移（含 env admin→首用户回填，upgrade/downgrade 往返通过）
...
- alembic 0017（§3.3）。生产执行前**单独确认**（C-1：破坏性/结构变更），先 `pg_dump` 受影响表。
...
| 止损预警 Job 多用户化（若超阈）| 可能牵动 APScheduler + 通知收件人模型（跨 phase 大重构）| 实施启动核查定；超阈→V1.5-F + 三链 |
