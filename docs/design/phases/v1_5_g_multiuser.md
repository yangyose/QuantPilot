...
**【设计待定：每日管线的持仓保护】** 每日批 `DailyPipeline` 是全局单次跑（无登录用户上下文）。持仓保护（candidate_pool 对已持仓股的保护、止损预警 Job）原依赖全局唯一账户。多用户后，全局管线产出共享候选池/信号**不含**任何用户持仓视图；持仓保护与止损预警须改为**按用户**触发（登录时实时计算 / 或月末批遍历所有 active user 的账户）。本 phase 设计**先把全局管线与账户解耦**（管线不再读 account），持仓保护移到 **API 请求期按 `get_current_account_id` 实时叠加**；止损预警 Job 的多用户化（遍历 active users）作为 **G-4 子项**纳入本 phase（实施启动核查 2026-06-30 锁定，详见 §6.4）。
...
- **止损预警 Job 多用户化**（承 §2 待定，实施启动核查锁定 2026-06-30 = **本 phase 内实现**）：原止损预警 Job 依赖全局唯一账户。多用户后改为遍历所有 `is_active` 用户的账户分别算回撤/止损并各自站内信（`in_app_notification` 已按 account 隔离）。实现要点：Job 函数从"读全局唯一账户"改为 `for user in active_users: account = get_account(user); compute_stoploss(account)`；通知收件人天然随 `account_id` 落各用户站内信。不引入新通知收件人模型（站内信本就 per-account）。纳入 G-4 测试（INT 覆盖多用户各自止损独立）。
