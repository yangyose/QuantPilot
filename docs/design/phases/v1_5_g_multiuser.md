...
> 版本：v1.4（G-6 收尾：测试/冒烟/文档回写全兑现，2026-07-23）
> 状态：代码交付完成（G-1~G-6 全绿）；余生产部署批次（alembic 0018-0020，需 C-1 单独确认 + pg_dump 前置）
...
| v1.4 | 2026-07-23 | **G-6 收尾收口**：§1.3 实施启动核查清单余下 3 项全勾（user_config.user_level 存量规范化以 G-4a「CONFIG_KEY_LEVEL 代码真源 + 过滤不读 DB 列」更强方案收口；system_design §3/§6 实施回写 v1.12）；§9 补交付映射注（UT/INT 随 G-1~G-4 交付 + G-6 新增 e2e AUTH-03b 停用登录 401 + 冒烟 API-108~114 + 冒烟全文件 account_id 传参清理/API-42 契约改写）；§10 DoD 8 行全勾。门槛：unit+e2e 726 / integration 201 / ruff 0 / vue-tsc 0 / 冒烟受影响 27 条真机 PASS。状态改「代码交付完成」，余生产部署批次 |
...
- [x] （已提前兑现）SDD §1.1 多用户前向注 + §6.3 `All→L1` 层级标签映射规约 + §11 env admin 涉及文档枚举
- [x] 实施期校验 `user_config.user_level` 存量值均规范化为 `L1/L2/L3`（G-4a 以更强方案收口：`CONFIG_KEY_LEVEL` 代码真源、`/settings` 过滤**不读** DB `user_level` 列（历史值稀疏/硬编码 L2/曾写非法 "USER" 不可靠），upsert 每次按真源回写规范值——存量非法值无消费路径、随后续写入自愈，无需一次性数据订正）
- [x] 实施期改 `CLAUDE.md §4.6/§4.7` + `dev_setup.md` + `override_admin_password` autouse fixture 多用户化（§11；G-2b 兑现：fixture 退役 + 集成 `test_user` fixture + 四处文档/示例 env 注记）
- [x] 实施期回写 system_design §3/§6（G-6 兑现，system_design v1.12：§3 补 User/auth_service/signal_view_service/rate_limit 落位；§6 补 register/me 三行 + 多用户约定块 + 附带修正 §14-10 漏删的 POST /positions 行）
...
> **交付映射（2026-07-23 G-6 收口）**：UT/INT 各项已在 G-1~G-4 随切片交付
> （`test_int_user_multiuser` 6 例=INT-MIG/约束、`test_int_auth_service` 7 例=INT-REG/AUTH/LVL、
> `test_int_iso_ownership` 5 例=INT-ISO、`/settings` level 过滤=UT `test_config_key_level` +
> e2e SAPI-02b）；G-6 补齐 e2e AUTH-03b（is_active=False 登录 401）+ 冒烟 **API-108~114**
> （register 422/409 + me 401/200 + PATCH level 422/幂等 200，对生产安全——重复注册用
> 已存在用户名不写库、PATCH 写回当前值；限频命中 429 时按 skip 处理）+ 冒烟全文件
> account_id 传参清理（API-42 断言改为 token 推新契约）。
...
- [x] `user` 表 + `account.user_id` + alembic 0018 迁移（含 env admin→首用户回填，upgrade/downgrade 往返通过）（G-1）
- [x] JWT 带 user_id 身份；`get_current_user_id/user/account_id` 依赖；登录改 DB 查询（G-2a）
- [x] `POST /auth/register`（限频 + 密码强度 + 唯一）+ `GET/PATCH /auth/me`（G-2a 端点 + G-2b 限频/EmailStr 收口）
- [x] 全部账户层路由 account_id 来源切 token + ownership 404 防护（G-3，含 report 0019；backtest 仅登录门槛为 §5.1 锁定决策）
- [x] `user.level` 偏好：信号解释前端按 level 显隐 + `/settings` 按 level 过滤（G-4a 服务端过滤 + G-5 前端血缘视图按 level 默认展开层数/设置面板按 requiredLevel 显隐）
- [x] 前端注册页 + 路由守卫 + 层级切换 + 删 account_id=1（G-5：RegisterView + 双向守卫 + 设置页「个人资料」Tab level 选择器 PATCH /auth/me + 前端 account_id 传参全删）
- [x] 测试全绿（UT/INT/E2E/冒烟）+ ruff 0 + vue-tsc 0（G-6：unit+e2e 726 / integration 201 / ruff 0 / 前端 vitest 21 + vue-tsc 0 + vite build；冒烟 API-108~114 新增 + account_id 清理，受影响 27 条对本地 :5433 全栈真机 27/27 PASS）
- [x] 文档：SDD §1/§2/§16 + system_design §3/§6/§9 + roadmap V1.5-G 回写一致（SDD v1.4-r2 设计期已回写；G-6 补 system_design v1.12 §3/§6 实施落位 + roadmap V1.5-G 行交付状态 + §4 G-4 多账户 UI 切换去向改指本文档 §12 推迟表）
