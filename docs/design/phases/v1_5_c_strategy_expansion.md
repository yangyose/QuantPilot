...
> 版本：v0.19（C4 转正前置：回测喂不起 money_flow，2026-09-23）
...
| **v0.19** | 2026-09-23 | **C4 转正前置入档：回测喂不起 `money_flow`**（§6.7 首段 + §6.6 新增一条 DoD）。`BacktestDataBundle` 无该字段、引擎构造当日快照时不填 `MarketSnapshot` 的同名键（生产侧 `_build_market_snapshot` 填了）→ `MoneyFlowStrategy` 逐日两因子全 NaN、被 `Scorer.aggregate` 以一行 INFO 跳过。影子期权重 0 故结果不受影响，但权重一旦 > 0，回测会**静默按「少一个策略」的口径算**——属 CLAUDE.md §4.11 那一族。判据落 `tests/unit/test_backtest_feeds_weighted_strategies.py`：默认权重矩阵里凡「回测喂不起」的策略权重必须为 0（改 0.05 即红，已变异验证），并用 `money_flow ∈ _UNFED ⟺ 回测喂不起它` 把登记与现实双向绑定（第二个方向是冷启动评审补齐的——首版只独立断言「现实还没接」，清空 `_UNFED` 会让它恒过）。同批登记 roadmap V1.5-L（v5.4）与 CLAUDE.md §6。⚠️ 本行是评审指出「版本号又没随正文前进」后补的（v0.15/v0.16 记过同一失败模式）；首次插入还插错了行序（落在 v0.18 之前），同一轮评审一并抓出 |
...
- [ ] **转正前置**：回测 bundle 补 `money_flow`（加载 + 按日切片进快照），补完从
      `tests/unit/test_backtest_feeds_weighted_strategies.py::_UNFED` 删除该项 —— 见 §6.7 首段。
      **权重给非零之前必须先勾掉这一条**，否则回测静默按「少一个策略」的口径算
...
**C4 转正的硬前置：回测喂不起 `money_flow`**（2026-09-23 钉死）。`BacktestDataBundle` 没有
...
⚠️ 这条只拦默认矩阵；ICIR/用户覆盖是运行时状态，测试取不到。故 C4 转正的实际次序是
**先补 bundle（含按日切片进快照）→ 再谈权重**。
