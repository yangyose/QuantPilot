...
- ~~策略实例注册三处~~ **实测 17 处**（实施订正，2026-09-09）：实例化确实是这三处——`api/deps.py`、`pipeline/daily_pipeline.py::_cp2_scoring`、`services/backtest_service.py`——但「加一个策略要改的地方」远不止实例化，还有权重矩阵、正交化顺序、`candidate_pool` 两张表的列、日级 IC 产出、月度再平衡、配置快照，以及测试里硬编码的策略名与列表长度。**改法见 §5.3 第三项**（`core/strategy_registry.py` 单一事实来源 + 契约测试），不是把 17 处抄进文档。
...
- [x] 单测：因子方向正确（低波动股 `inv_volatility` 更高）—— **12 条**（`tests/unit/test_low_volatility.py`）
- [x] 单测：注册点齐全 —— **15 条**（`tests/unit/test_strategy_registry.py`）。⚠️ **§5.2 的「三处注册点」是错的，实际 17 处**：设计只数了策略实例化的 3 个地方，漏掉了权重矩阵、正交化顺序、`candidate_pool` 列、IC 产出、月度再平衡、配置快照、以及若干测试里硬编码的策略名/列表长度。改法不是把 17 处列进文档（下次加策略照样漏），而是新建 `core/strategy_registry.py` 作**单一事实来源**（`STRATEGY_NAMES` + `SCORE_COLUMN_MAP`，后者容纳 `mean_reversion → reversion_score` 这个不规则映射），让引用点从常量派生；派生不了的（DB 列、迁移）由契约测试钉死
- [x] §8 的 composite 接入项全部通过（影子模式默认 0 权重、正交化矩阵排除、`candidate_pool` 扩列 = alembic **0029**，两张表都要改）
- [x] 集成测试：断言现有四策略 `z_raw` 与接入前**逐值一致** —— **7 条**（`tests/unit/test_low_volatility_zero_regression.py`）
