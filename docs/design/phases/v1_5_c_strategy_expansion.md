...
> 版本：v0.16（头部状态行回写至真实交付状态，2026-09-02）
> 状态：**C0 全量上线**（2026-08-19 六步生产收尾逐步实证，alembic 至 0025，`daily_ic_producer` 19:30 Job 已激活并完成首跑）；**C1 全部已上生产**（2026-08-31，与 P0 退出修复同批，生产 = `2bab523`）——C1-1 约束落点统一 `ac069e5` / C1-2 风险调整动量 `85df015` / C1-3 价格窗口按交易日推导 `be6d6d6`；**C1 面板对比已完成**（2026-08-28，off 5h10m / on 4h57m，497 交易日 × 4 策略，结论见 §3.3——**上线理由是 C1-1 + C1-3 两个缺陷修复，不是 C1-2 验证有效**）；**C2 代码六块已完成五块**（Engine 纯函数 / 数据层 7 列 + alembic 0028 / 两条采集路径 / 同比配对取数 / 门控 + Service 接线），**差最后一步「5y 回填激活」**——回填前 7 列全 NULL → F-Score 全判「不可判」→ 门控不生效（设计内的可见降级，`piotroski_gate_skipped` 有日志）。C3-C5 待启动。scope 锁定 C0-C5 六子批、零推迟
>
