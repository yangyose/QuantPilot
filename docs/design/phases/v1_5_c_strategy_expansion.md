...
> 三处都记着已完成——**同一份文档内部头尾相反**。头部横幅是读者第一眼看的东西，
> 它比正文更需要准确。判据：改动 §3.3 或任一子批 DoD 时，回头读一遍本行。
...
| v0.14 | 2026-08-28 | **C1 面板对比完成 + §3.3 结论回写**（C1 最后一项 DoD 收口）。两组各 25 chunk 全 `rc=0`，497 交易日 × 4 策略 = 1988 行，off 5h10m / on 4h57m。**前提证伪检查通过**：trend/value/mean_reversion 逐日 IC **逐值相同**（差异恒 0）→ momentum 的差异可干净归因到 C1-2，且证明管线确定性、开关无泄漏。**结论（如实记录，不设转正门槛）**：方向一致为正但幅度微小——OSCILLATION（占 65% 天数）+0.0007 `t_HAC=0.19` 属噪声；UPTREND +0.0078 `t_HAC=1.72` 不显著；DOWNTREND +0.0084 `t_HAC=2.32` 名义显著但仅 70 天、非重叠样本 4 个无法检验、系三重检验之一。**momentum 三状态仍为显著负 IC（−0.085 / −0.069 / −0.016），C1-2 未改变其符号或性质**；ICIR 已把 momentum 权重压到 0，故实际影响接近于零。**方法学**：重叠窗口使有效样本仅名义的 8~12%，朴素 t 与 HAC 差约 3 倍（6.83 → 2.32），只看朴素 t 会得出「三状态全部高度显著」的错误结论（⚠️ **这句本身即过度概括，见 v0.15**：OSCILLATION 朴素 t 仅 0.65，本就不显著；实为 UPTREND / DOWNTREND 两状态被误判）；中途快照（18/25）时 OSCILLATION 为 +0.0033/1.33，全程收敛至 +0.0007/0.19，故中途数字不可当结论。**对上线的定性**：C1 批次应上线，但理由是 **C1-1（价值陷阱护栏失效，value 权重 0.57~0.87）+ C1-3（`rs_6m` 0/2274）两个缺陷修复**，**不是 C1-2 有效**——不得记述为「风险调整动量验证有效」，否则后续扩展将建立在错误前提上 |
...
> 🔴 **2026-09-07 追加更正：这一节的成因判断只对了一半，范围也远小于实际。**
> 本节把 F-4 的切换归因于 `total_equity` 在**数据里**稀疏，但真正的根因在**查询侧**：
> `get_latest_financial` 用一个共用报告期供全部字段取值，而 `total_equity` 每股每期
> 只有一条锚点行、其余字段每条日频行都有 → 报告期一滚，新期就把有值的旧期整个挡掉。
> 且失效范围远不止「窗口中段」：实测 5 年 21 个采样日中 **10 天**覆盖率低于 19%。
> 之所以看起来只是「中段切换」，是因为那条 WARNING 写的是 `isna().all()`，
> **只在恰好 100% 全 NULL 时才响**——它分辨不出 1.2% 与 99%。
> 已修：`6bdd623`（按字段 LOCF）+ `1b05bc8`（改按覆盖率阈值告警，日志键改为
> `universe_filter_low_coverage`）。详见 V1.5-K 设计文档修订历史 v0.11 ③。
> ⚠️ **C1 面板对比（§3.3）产出于修复之前**，其口径断点由本节描述，结论不因此变更
> （off/on 两组同批同口径），但与修复后数据的任何对比都跨了这个断点。
...
> 🔴 **上面这张表是 2026-08-26 的快照，末句现在是假的（2026-09-07 订正）。**
> 2026-09-05 的 `81b755b`（`scripts/backfill_total_equity.py`）已对 5434 与生产双边
> 回补 2021-03-31~2024-03-31 各报告期（由 0 补至 4378~5594）。当前实测：
> **2023 全年 21413**（表中写 225）、**2024 全年 21357**（表中写 16067）。
> 故今天从任一新备份重建算力库，得到的**不是**表中那个稀疏状态——「结果相同」不再成立。
> 表格保留是因为它记录了**当时**的测量，删掉会让下方「两点后果」失去依据；
> 但任何按本表推断**当前**数据状态的做法都会出错。
...
- [x] 本地 5y 面板对比：改造前后 momentum 日级 IC / ICIR（OSCILLATION + UPTREND）。**2026-08-28 完成，结论见下方「§3.3 结论」**——momentum 三状态仍为显著负 IC，C1-2 改善微小且在主导状态下不显著。**要求记录真实结果，不设"必须转正"的硬门槛**——若仍为负则如实记录并在 §3.3 结论中写明（负 IC 本身是有效信息，ICIR 机制会继续给 0 权重；禁止为达标调参硬凑）。**对照基准须为 C1-3 修复后的 `off` 组**，不得用 `ic_baseline_pre_c1` 快照（该快照产出于窗口修复之前，momentum 的 `rs_6m` 是全 NaN 的，见 §3.2 C1-3）。回写结论时**必须一并写明 `total_equity` 口径断点**（§3.2 面板口径小节）：窗口中段 F-4 由跳过转为生效，绝对 IC 水平不可外推到生产
...
### 3.3 结论：C1 面板对比实测（2026-08-28）

**跑法**：第二台 24h 算力机 5434，窗口 2024-07-01 ~ 2026-07-17（**497 个交易日 × 4 策略 =
1988 行**，两组各 25 个月度 chunk 全部 `rc=0`）。off 组 5h10m，on 组 4h57m。
基准为 **C1-3 修复后的 `--momentum-risk-adjusted off`** 组（非 `ic_baseline_pre_c1` 快照，
理由见 §3.2 C1-3）。

#### 前提：证伪检查通过

`--momentum-risk-adjusted` 只应改 momentum。逐日 IC 取自**正交化之前**的
`score_breakdown_raw[strategy]["z_raw"]`，故其余三策略应逐值相同。实测：

```
trend / value / mean_reversion   各 497 天   非零差异 0
```

三者**逐值完全相同** → momentum 的差异可干净归因到 C1-2，且证明管线确定性、开关无泄漏。

#### 配对结果（diff = IC_on − IC_off）

两组跑同一批交易日、同一 universe，故按 `(strategy, state, trade_date)` 配对，
可消掉共同市场噪声、功效远高于比较两个独立均值。

| 状态 | 天数 | IC_off | IC_on | 差值 | **t_HAC20** | t_朴素 | 有效样本比 |
|---|---|---|---|---|---|---|---|
| OSCILLATION | **323**（65%） | −0.0700 | −0.0693 | **+0.0007** | **0.19** | 0.65 | 0.08 |
| UPTREND | 104 | −0.0241 | −0.0162 | +0.0078 | **1.72** | 4.91 | 0.12 |
| DOWNTREND | 70 | −0.0931 | −0.0847 | +0.0084 | **2.32** | 6.83 | 0.11 |

**判读（如实记录，不设转正门槛）**：

1. **方向一致为正**——风险调整动量确实让 momentum 的 IC 向 0 靠近，三个状态无例外。
2. **但在占 65% 天数的 OSCILLATION 上几乎无效**：+0.0007，`t_HAC = 0.19`，属噪声量级。
3. **DOWNTREND 名义显著（2.32）但证据薄弱**：仅 70 天（约一个下跌周期），非重叠样本仅 4 个
   无法检验，且系三个同时检验之一。
4. **momentum 三状态仍为显著负 IC**（−0.085 / −0.069 / −0.016）。**C1-2 是在一个负因子上做了
   小幅改善，未改变其符号或性质。** 由于 ICIR 已把 momentum 权重压到 0，该改善在生产的实际
   影响接近于零。

#### 必须连同记录的三条口径限制

- **`total_equity` 口径断点**：窗口 2024-07 → 2026-07 **中段存在一次 universe 口径切换**
  （F-4 由跳过转为生效，因 `total_equity` 仅覆盖 2025-06-30 起 5 个报告期，见 §3.2 面板口径）。
  ⚠️ **该成因表述已于 2026-09-07 被推翻**：历史锚点行已由 `81b755b` 回补，真正的根因在
  查询侧（共用报告期挡掉稀疏字段，`6bdd623` 修复）。**口径断点这个事实不变**——C1 面板
  产出于两个修复之前，故上面这句仍是本次对比的真实限制；但不要再拿它去推断当前数据状态。
  **绝对 IC 水平不可外推到生产**；off/on 差值不受影响，那正是本次要测的量。
- **重叠窗口必须修正**：前向窗口 20 交易日、逐日计算 → 相邻日收益区间共享 19/20，
  **有效样本仅名义的 8~12%**。朴素 t 与 HAC 相差约 3 倍（6.83 → 2.32），
  **只看朴素 t 会把 UPTREND（4.91）与 DOWNTREND（6.83）双双误判为「高度显著」**，
  而 HAC 后分别退到 1.72 / 2.32。已据此登记 roadmap V1.5-K。
  ⚠️ v0.14 此处原写「三状态全部高度显著」，**是过度概括**：OSCILLATION 的朴素 t 仅
  **0.65**，它在朴素法下本就不显著，谈不上"退位"（2026-09-02 第三方评审订正）。
  该错误曾被 `v1_5_k §1.1` 原样引用——**上游一句话的松散，下游会照抄**。
- **中途快照不可当结论**：跑到 18/25 时 OSCILLATION 差值为 +0.0033（`t_HAC` 1.33），
  全程跑完收敛为 +0.0007（0.19）——效应随样本补全向 0 收敛，本身即说明其在噪声量级。

#### 对 C1 上线的定性（重要）

**C1 批次应当上线，但理由是两个缺陷修复，不是 C1-2 有效：**

| 子项 | 性质 | 价值 |
|---|---|---|
| **C1-1** 约束落点统一 | **缺陷修复** | 价值陷阱护栏在五步管线里长期失效，而 value 占 composite 权重 0.57~0.87 |
| **C1-3** 价格窗口按交易日 | **缺陷修复** | `rs_6m` 有效率 0/2274 → 2246/2274，自 Initial commit 起的残缺 |
| C1-2 风险调整动量 | 增强 | **实测微小、主导状态不显著** |

**不得将本次结论记述为「风险调整动量验证有效」**——那会让后续基于该结论的扩展建立在错误前提上。

#### 产物位置

| 产物 | 位置 |
|---|---|
| off / on 原始 IC 行 | `backups/local-5434/panel_ic_off_20260827_183238.sql` / `panel_ic_on_20260828_154213.sql` |
| off / on summary | `backend/var/diagnostics/c1_panel_off/` / `c1_panel_on/` |
| 面板前生产 IC 行（含已删的 28 行污染） | `backups/local-5434/prod_ic_rows_20260827.sql` + `prepanel_prod_ic_rows_20260718_0731.csv` |

> 配对分析（HAC / 非重叠 / 证伪检查）目前是 scratchpad 一次性脚本，**尚未入仓**，
> 已登记 roadmap V1.5-K 一并收编。
...
- 落点：`MeanReversionStrategy.apply_constraints`（C1-1 的钩子）。命中门控（`f_score < piotroski_min_score`，默认 6）→ 该行 `rsi_oversold / price_deviation / bb_position` **三列全置 NaN**。
- ⚠️ **默认影子模式**（`MeanReversionStrategyConfig.piotroski_gate_enabled = False`，2026-09-09 加）：上述三条分支照常计算、照常记日志（`piotroski_gate_shadow: blocked=N ... threshold=6.0`），但**不改数据**。开发集实测门控在任何阈值上都无显著收益（§4.5 与 `docs/reviews/scoring_monotonicity_2026-09-09.md` §7）；机制先验合理故保留观察，而不是删掉。开关与阈值都由 4 条「改开关/改阈值 → 结果必须变」的测试钉死（4 变异全拦：阈值写死 / 开关写死 True / 开关写死 False / 日志不区分 shadow-applied），避免变成 §4.11 第 2 例那种「对用户可编辑但代码永不读」的平行副本。
...
- ⚠️ **「提前激活」陷阱**（2026-09-09 生产首跑实测发现）：金融股的 ROE 替代分支读
  ...
  **激活顺序：先回填 7 列 → 确认 `judged` 接近 universe → 再谈激活**；
  ...
  ✅ **当前无风险**：`piotroski_gate_enabled` 默认 **False**（本节上一条），
  所以这 71 只只是被记进日志、并未真的被剔除——上面说的是「激活时会怎样」，不是现状。
- **Altman Z-Score 备选：已决定放弃**（2026-09-09，依 v0.16 预设的 <5% 判据就地裁定，未跨子批推迟）。5434 全量回填后逐年实测「不可判占比」：

  | 采样日 | 股票 | 可判 | 不可判 | 不可判占比 |
  |---|---|---|---|---|
  | 2022-06-15 | 4826 | 4259 | 567 | **11.7%** |
  | 2023-06-15 | 5207 | 4759 | 448 | 8.6% |
  | 2024-06-15 | 5364 | 5151 | 213 | 4.0% |
  | 2025-06-16 | 5418 | 5230 | 188 | 3.5% |
  | 2026-08-25 | 5515 | 5384 | 131 | **2.4%** |

  裁定理由：门控是**前瞻使用**的机制，其成本收益只由「今天及以后的不可判占比」决定——那是 2.4~4.0%，稳定低于 5% 判据。2022~2023 的 11.7%/8.6% 不改变结论，因为它不是「这些公司真的无法判定」，而是 `fina_indicator` 对更早报告期的字段覆盖本身就低；为 5 年前的数据引入第二套判据（Altman 需要 EBIT / 留存收益 / 市值三组新字段 + 一套独立阈值），只为让回测里那几个百分点从 fail-open 变成被门控，**复杂度与收益不成比例**。
  ⚠️ 该结论绑定「不可判 = 不门控」这一 fail-open 语义（本节上一条）。若将来把不可判改成门控，Altman 必须重新评估——那时 2022 的 11.7% 就直接等于误杀率。

  逐项可判率（2026-08-25，9/9 项全部可判，`total_share` 回填后）：`cfo_positive`/`accrual_quality` 99.9%、`leverage_down` 99.5%、`turnover_up` 99.4%、`roa_positive` 98.2%、`roa_improved`/`margin_up` 98.1%、`liquidity_up` 97.8%、`no_dilution` 97.2%。
- 可观测：`piotroski_gate_applied: date=%s blocked=%d unjudgeable=%d financial_alt=%d`（同 A5b 先例，激活可实证）。

### 4.5 C2 DoD

- [x] RED：`compute_f_score` 9 项逐项单测（含边界：ε 容差、全字段齐 → 0~9、缺 1~2 项 → 部分可判、缺 ≥3 项 → NaN）—— **20 条 / 4 变异全拦**（`tests/unit/test_piotroski.py`）。⚠️ 逐项断言写法有过一次订正：原写「改一个字段只有对应那项变」，但 `ocfps` 同时喂 #2 与 #4，正确的不变量是**变化不超出该字段的依赖集**
- [x] 单测：门控在 `compute_strategy_factors` 生效（命中行三列 NaN）；金融股走 ROE 替代；不可判不门控 —— **11 条 / 4 变异全拦**（置 0 而非 NaN / 不可判也门控 / 金融股不走替代 / 门槛写成 7）。另加 **AST 调用点断言**：`ScoringService._build_market_snapshot` 必须真的取同比配对、算 F-Score、把 `f_score` 与 `stock_info` 写进快照——否则门控写对了也永不生效（§4.11 表第 4 例形状）
- [x] alembic **0028**（纯 `ADD COLUMN`，前向非破坏）；ORM 7 列与迁移一致，5433 upgrade 实跑通过
...
- [x] `get_financials_yoy_pairs` 集成测试：PIT 约束 + 同比而非环比 —— **7 条 / 3 变异**。⚠️ 首版 5 条**没拦住「去掉 HAVING」那个变异**，因为测试里没有「未披露占位行」这种输入，而它恰是生产常态（2026-08-25 实测 87% 的股票最近一期就是占位行）。补 `test_yoy_06` 后才拦住——**「测试输入比现实更配合」（§4.11 第 7 类）这次出在本批新写的测试上**
- [x] **两条采集路径都扩了字段**：日频 `fetch_financial_data`（含 `daily_basic.total_share`，万股 → 股）与逐股 `fetch_financial_by_stock`（`publish_date = ann_date`，回填走这条）。只改一条 → 回填出来的历史 6 列全 NULL → F-Score 全历史「不可判」→ 门控在回测/面板里永不生效且零报错
- [x] **实施中撞到两层白名单**，两层都会**静默丢字段**（今日第三次遇到该形态，前两次是早上的 `get_latest_financial` 共用报告期）：① 适配器 `fina_cols`（merge 白名单）→ 字段进不了 DataFrame；② `_FINANCIAL_UPDATE_COLS`（upsert 白名单）→ 字段进不了库。两处均已补齐并加注释「加字段务必同时改这里」，后者由 `test_yoy_07` 以**查库读回的值**为判据钉死（不是「upsert 没抛异常」——那在缺陷仍在时恒真）
- [x] **5y 回填（本地算力中心 5434）已完成**，7 列非空率按 `(ts_code, report_period)` 粒度实测 **94.7~99.9%**（2021-03-31 起 22 期；节选：2025-12-31 期 `ocfps`/`eps`/`assets_turn` 99.9%、`roa`/`grossprofit_margin` 98.0%、`total_share` 98.8%）。⚠️ **不要按行统计非空率**——`financial_data` 每个 (股票,报告期) 有约 60 个 publish_date 快照行，回填只写公告日那一行，按行算会得到 1.6% 这种看着像「回填失败」的数字；读取路径（`get_financials_yoy_pairs`）是 `max(...) GROUP BY (ts_code, report_period)`，上面的粒度才与它一致
- [x] **回填必须逐期调用**（`--period` 循环），不能按日期窗口批量：`fina_indicator` 的日期窗口形态有 **100 行硬上限**，5 码跨 5.7 年恰好返回 100 行且不报错。首版脚本因此报 `ok=5515 fail=0` 却只填了最新一期。守卫已加在 `TushareAdapter._call`（`_warn_if_row_capped`），判据见 CLAUDE.md §4.3
- [x] `total_share` 走 `daily_basic`（每个报告期末交易日一次调用，万股 → 股）。回填前 `no_dilution` 恒不可判 → 9 项只有 8 项可判 → F-Score 中位数被压低 1 分（4.0），门控率虚高 **76%**；回填后 9/9 可判、中位数 5.0、门控率 **62%**。这一项本身就是「缺数据伪装成基本面恶化」的实例（§4.3 那条设计约束的现实验证）
- [x] 不可判占比实测 → Altman Z-Score 备选**已决定放弃**，逐年数据与理由写回 §4.4
- [ ] 生产 5y 回填（C-1 授权 + 前置 `pg_dump -t financial_data` 定点备份）——**与 C2/C3 代码同批部署时执行**，生产当前 = `85438a8`（alembic 0027），尚不含 0028 的 7 列
- [ ] 生产实证日志 `piotroski_gate_applied` 出现且 `blocked > 0`
- [x] **门控阈值 `f_score < 6` 实测复核已完成 —— 结果是负的，据此决定「上线但不激活」**（2026-09-09，开发集 28 个采样日、窗口不重叠、h=20；完整表见 `docs/reviews/scoring_monotonicity_2026-09-09.md` §7）：策略 IC 在 6 个阈值上基本不动（0.0498~0.0634，t 全在 1.8~2.2），头部 5% 超额**不单调**（F<5 反比 F<4 更差），且**无一显著**（|t| ≤ 1.13）。F<7 的 +0.0005 看着转正但 t=0.09，代价是丢掉 72% 可选池。**处置：门控代码照常上线，但只计算、只记日志（`piotroski_gate_applied`），不真正剔除。**机制先验合理（均值回归买下跌股，避开基本面恶化的公司＝避免接飞刀）值得留在生产观察，但先验不能替代证据——开发集测不出来就激活，等于在下一个子批里自己推翻 §3.3 对 C1-2 立的规矩。**再评估条件**：holdout（2025-08~2026-07）+ 生产影子期数据独立复现同向改善后，再讨论激活与阈值
- [ ] 影子期后按上条的再评估条件决定是否真正激活门控（**不设「必须转正」门槛**）
- [ ] SDD §7.2.2 回写
...
- [x] RED：σ60 / β120 纯函数单测（含已知解析解的构造数据、样本不足 → NaN、常数序列 → NaN 而非 0）—— **12 条**（`tests/unit/test_volatility.py`）。`rolling_beta` 在 `var(r_m) == 0` 时返回 **NaN 而非 inf**：常数基准序列除零会得 ±inf，而 inf 能一路穿过 Winsorize 与 Z-score 不报错
- [x] 单测：因子方向正确（低波动股 `inv_volatility` 更高）—— **15 条**（`tests/unit/test_low_volatility.py`）
- [x] 单测：注册点齐全 —— **15 条**（`tests/unit/test_strategy_registry.py`）。⚠️ **§5.2 的「三处注册点」是错的**——实例化就有四处（漏了 `scoring_factory.py`，面板/回填走的正是那条），而「加一个策略要改的地方」还包括名单五处、权重矩阵、`CompositeScore` 字段与 `Scorer.aggregate` 赋值行、`PoolEntry`、`candidate_pool` 四处写入 dict、`signal_service`、`schemas/signals.py`、两张表的 DB 列：设计只数了策略实例化的 3 个地方，漏掉了权重矩阵、正交化顺序、`candidate_pool` 列、IC 产出、月度再平衡、配置快照、以及若干测试里硬编码的策略名/列表长度。改法不是把清单列进文档（抄一份就多一份必然漂移的副本），而是新建 `core/strategy_registry.py` 作**单一事实来源**（`STRATEGY_NAMES` + `SCORE_COLUMN_MAP`，后者容纳 `mean_reversion → reversion_score` 这个不规则映射），让引用点从常量派生；派生不了的（DB 列、迁移）由契约测试钉死
...
- [x] 🔴 **`low_volatility_score` 全链路死代码——第三方评审抓出，已修**（2026-09-09）。上一条打勾时它其实是假的：`STRATEGY_NAMES` 登记了、`SCORE_COLUMN_MAP` 映射了、`PoolEntry` 有字段、alembic 0029 给两张表都加了列、契约测试断言列名存在——**而 `Scorer.aggregate()` 从头到尾没把值填进去**（`_scalar("trend"/"momentum"/"mean_reversion"/"value")` 四行都在，唯独漏了第五行），`candidate_pool` / `signal_score_snapshot` 的新列因此永远是 NULL。**1184 条测试全绿**，因为没有一条问过「值到了吗」。这是 CLAUDE.md §4.11 表第 4 例的同型。
  ...
  **护栏 6 条**（`tests/unit/test_strategy_score_reaches_db.py`，**遍历 `STRATEGY_NAMES` 而非写死五个名字**，故加第六个策略时同样会红）：①映射完整性；②每个策略的分数都被 `aggregate` 填入；③**改该策略的因子 → 它的分数必须变**（只断言「非 None」拦不住「接到了别的策略上」，实测该变异确实需要这条才红）；④**AST 检查候选池行 dict 的键**——按 §4.11「调用点是否真传参」，构造 `PoolEntry` 再调写入函数是自证式的；
  ...
  **以上 ①~⑥ 共 6 条**，变异全部验过：撤掉 `aggregate` 赋值行 / 接到 `value` 上 /
  撤掉一处候选池 dict 键 / 凭空加一列不接线 —— 各自被对应那条拦住。
  （⚠️ 本段曾把 ①~④ 与 ⑤⑥ 拆成三个自然段、且把变异说明放在 ④ 末尾，
  读起来像「只有 4 条」——冷启动评审员就先误读成 4 条，故改为在此统一收口。）
...
- [ ] 影子期结束后决定是否给非零权重（**不设「必须转正」门槛**，沿用 §3.3 对 C1-2 的同一纪律）
...
- [x] C-1 授权 → 2y 回填（先 5434，再生产）→ 回填后核查覆盖率与磁盘占用并记录 ——
      **回填 2026-09-16/17 完成，审计 2026-09-23 补做**（此前只勾不了这条是因为审计没做，
      不是回填没做）：5434 2024-08-26~2026-08-25 / 484 交易日 / 2,496,170 行 / **541 MB**；
      生产 2024-09-18~2026-09-22 / 489 交易日 / 2,530,854 行 / **540 MB**（vs §6.4 外推 0.57 GB），
      两边**对交易日历零缺日**，每日 5084~5554 行。⚠️ 覆盖率见下条
- [x] **转正前置**：回测 bundle 补 `money_flow` —— **2026-09-23 已交付**（bundle 字段 +
      Service 复用生产 `get_money_flow_window` 拉宽窗口 + 引擎 `_money_flow_at` 逐日 PIT 切片，
      `_UNFED` 已清空）。5434 实测两日逐股逐因子与生产取数路径**完全相同**
      （5d 有值 5502/5515、20d 5188、max|Δ|=0）；6 日回测 `scorer_strategy_skipped_all_nan:
      strategy=money_flow` 由每日一条降为 **0 条**，指标不变（影子权重 0 的数学保证）。
      判据见 `tests/unit/test_backtest_money_flow.py` 与 §6.7 首段
- [ ] ⚠️ **数据边界：历史覆盖缺北交所**（2026-09-23 审计发现，**转正评估的前提条件**）。
      历史日 `money_flow` 对当日有行情股的覆盖率约 **95%**，缺口**全部是 BJ**：2026-03-31 缺 302 只、
      当日 BJ 共 303 只；按月看 BJ 在 2026-07 及以前**每天仅 1 只**，2026-08 起才全量（约 336/日）
      → Tushare 的 BJ 资金流历史实质自 2026-08 起。而 BJ 并非被流动性挡掉的边角：09-22 有
      310/345 只过 F-7 阈值（5M 元）、候选池 68 只里占 **10 只**。
      ⇒ **C4 的历史 IC / 回测因子在 BJ 子集上恒 NaN、无观测**；K-7 或任何转正判断都必须声明
      这个子集缺口，或等 BJ 历史可得后重算。此条不影响影子期（权重 0），故不阻塞部署
...
**C4 转正的硬前置：回测喂不起 `money_flow`** —— ✅ **同日（2026-09-23）已补**，下文是成因与
现在的口径。~~`BacktestDataBundle` 没有
`money_flow` 字段；`MarketSnapshot` 这个 TypedDict **有**该键（生产侧要用），但引擎构造当日
快照的字典字面量里**不填它** → `MoneyFlowStrategy` 在回测里逐日
两个因子全 NaN → `Scorer.aggregate` 记一行 `scorer_strategy_skipped_all_nan` 并跳过。影子期
权重 0，结果不受影响；但权重一旦 > 0（月末 rebalance 自动激活或手工改矩阵），**回测会静默
按「少一个策略」的口径算**，日志里只有一行 INFO——正是 CLAUDE.md §4.11 那一族。
判据落在 `tests/unit/test_backtest_feeds_weighted_strategies.py`：默认权重矩阵里凡「回测喂不起」
的策略权重必须为 0（把 `money_flow` 改 0.05 即红，已变异验证），且反向钉死「money_flow 现在
确实不在 bundle 里」——补完 bundle 后从 `_UNFED` 删除该项，两条同时松开。~~
⚠️ 这条只拦默认矩阵；ICIR/用户覆盖是运行时状态，测试取不到——所以它只是「谁想跳过前置」的
最后一道闸，不是前置本身。

**现在的口径（2026-09-23 交付）**：`BacktestDataBundle.money_flow` 存 long 表，Service 用
**生产那条 SQL**（`get_money_flow_window`，含 INNER JOIN `daily_quote` 取 `amount`）把窗口
拉宽成 `[start − lookback, end]` 一次取回；引擎 `_money_flow_at` 逐日切 `[td − lookback, td]`
（两端闭，与 SQL 相同）并保持 (ts_code, trade_date) 升序——**上下界都不能省**：
少了上界 `<= td` 就是前视（策略靠 `tail(N)` 取最近 N 行），少了下界则长期停牌股在生产是
NaN（窗口内行数不足）而回测拿到窗口外老行反而有值。`lookback` 由
`resolve_money_flow_lookback_days(strategies)` 从策略配置读，与生产同源（取不到回落 40）。
实测（5434，2026-08-18 / 08-25 两日，universe 5515）：回测切片与生产取数路径的**行数、
逐股逐因子值完全相同**（5d 有值 5502、20d 5188、max|Δ| = 0）；6 日回测 `money_flow` 的
`skipped_all_nan` 由每日一条降为 0 条，`max_drawdown` 0.013136 **不变**（影子权重 0 的数学保证，
即 §8.2「加 0 权重项不改变现有相对权重」在回测侧的实证）。判据 `tests/unit/test_backtest_money_flow.py`
（含「不设下界就会拿到值」的反证与三个调用点 AST 钉）。
`_UNFED` 已清空——⚠️ 因此 `test_unfed_strategies_must_have_zero_default_weight` 现在是**空循环恒真**，
把关职责整个落到 `test_unfed_list_matches_reality`（结构性判据：bundle 有字段 **且** 引擎快照里
真填了这一键 ⟺ `_UNFED` 不登记它）。下一个读者别以为两条等效：前者是「将来有新策略喂不起时」
的闸门，后者才是现在还在防守的那条。
