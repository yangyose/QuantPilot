...
> 版本：v1.1（2026-05-25 修订）
> 状态：设计完成 + 评审通过（v1.0 评审 P1×3 + P2×4 + P3×6 全收口） → TDD 实施
...
> - 评审报告：`docs/reviews/phase14_design_review_2026-05-25.md` v1.0（本次修订依据）
...
| 14-3 | BacktestEngine 真 5 步管线接入（采方案 A，engine 层抽纯函数） | 1-1.5 | §5 |
...
| 孤儿模块检查（system_design §3/§5）| Phase 14 不新增模块；engine 层新增 `engine/scoring/pipeline.py` 纯函数（属 §14-3 方案 A）→ system_design §3 待同步收口时追加 |
...
### 5.2 实施路径（方案 A：engine 层抽 5 步纯函数）

**5.2.1 架构事实核查（基准 commit `9ff9cb3`）**

- `engine/backtest/engine.py:121` `BacktestEngine.__init__(scorer: Any)` 实际持有的是 `engine.scorer.Scorer` 实例（engine 层），**没有** `score_universe` 方法（该方法定义在 `services/strategy_service.py::ScoringService:399`，是 async + 需 AsyncSession + 内部走 `factor_monitor.get_active_weights(session, ...)`）
- CLAUDE.md §6 "Engine 层严格无 IO" → BacktestEngine 不能直接持有 ScoringService

**结论**：必须把 Phase 11 5 步管线的**算法核心**从 ScoringService 抽出为 engine 层纯函数，让 BacktestEngine 通过 outer session 预查询好所有依赖数据后调用纯函数。

**5.2.2 engine 层新增 `engine/scoring/pipeline.py`**

```python
# 纯函数签名（待实施时与 ScoringService.score_universe:399-… 既有 5 步代码对齐验证）
def run_scoring_pipeline(
    strategy_factor_dfs: dict[str, pd.DataFrame],   # {strategy: factor_df}
    market_state: MarketStateEnum,
    weights_snapshot: dict[str, float],              # active_weights snapshot
    industry: pd.Series,                              # ts_code → industry_l1
    market_cap: pd.Series,                            # ts_code → 流通市值
    *,
    config: ScoringPipelineConfig,                   # winsorize / 中性化 / Z / GS / 三层 阈值
) -> CompositeResultDataFrame:
    """5 步管线纯函数：Winsorize → 行业+市值中性化 → Z-score → Gram-Schmidt → 三层输出。

    输入全部预查询完毕，输出 composite_z / composite_pct_in_market / strategy_z / factor_neutralized
    （三层）；不持 session，不读 DB。

    `len(universe) < WINSORIZE_MIN_SAMPLES` → 抛 `InsufficientUniverseError`
    （供调用方降级 aggregate_legacy）。
    """
```

**WINSORIZE_MIN_SAMPLES**：从 ScoringService 既有常量 import（避免硬编码 30）。

**5.2.3 `Scorer` 加公开方法 `aggregate_pipeline(...)`**

封装对 `engine/scoring/pipeline.py::run_scoring_pipeline` 的调用 + 异常处理：
    ...
    def aggregate_pipeline(
        ...
        try:
            return run_scoring_pipeline(
                strategy_factors, market_state, weights_snapshot,
                industry, market_cap, config=config,
            )
        except InsufficientUniverseError:
            raise  # 让调用方决定是否降级 aggregate_legacy
```

**5.2.4 BacktestService 预查询 5y 数据**

`BacktestService.run_backtest` 在 outer session 内预查询：
- 5y `active_weights` 时序（`factor_monitor.get_active_weights_history(start, end)` 新增）→ `BacktestDataBundle.active_weights_by_date: dict[date, dict[str, float]]`
- 5y `industry` / `market_cap` PIT 切片 → `BacktestDataBundle.industry_by_date` / `BacktestDataBundle.market_cap_by_date`

**5.2.5 BacktestEngine 主循环改写**

```python
# engine/backtest/engine.py::run 主循环每个 trade_date
weights = bundle.active_weights_by_date.get(trade_date)
industry = bundle.industry_by_date[trade_date]
market_cap = bundle.market_cap_by_date[trade_date]
universe = list(strategy_factors[list(strategy_factors.keys())[0]].index)

if len(universe) < WINSORIZE_MIN_SAMPLES or weights is None:
    # 降级：universe 不足 / weights 未就绪 → 走 legacy + 派生 z
    result = self._scorer.aggregate_legacy(snapshot, market_state)
    composite_z = derive_z_from_score(result.composite_score)
    pipeline_mode = "legacy_fallback"
else:
    try:
        composite_df = self._scorer.aggregate_pipeline(
            strategy_factors, market_state, weights, industry, market_cap, config=cfg,
        )
        composite_z = composite_df["composite_z"]
...
# BacktestResult 写 pipeline_mode 供前端展示
```
...
| 编号 | 类型 | 覆盖 |
|------|------|------|
| UT-P14-3-01 | unit | engine/scoring/pipeline.py::run_scoring_pipeline 5 步算法对齐 ScoringService.score_universe 既有逻辑（合成 50 universe + mock industry/market_cap） |
| UT-P14-3-02 | unit | Scorer.aggregate_pipeline universe < WINSORIZE_MIN_SAMPLES → 抛 InsufficientUniverseError |
| UT-P14-3-03 | unit | BacktestEngine run universe ≥ 30 + weights 就绪 → pipeline_mode='real_5step'，composite_z 落 N(0,1) ±3.5σ |
| UT-P14-3-04 | unit | universe < 30 / weights=None → pipeline_mode='legacy_fallback' |
| INT-P14-3-01 | integration | 真 DB 跑 30 trade_date × ~2400 universe 回测，composite_z 分布断言 + pipeline_mode 统计 |
...
§14-2 5y candidate_pool 回填 + ICIR 历史回算（耗时最长 50-80h）
       ↓
§14-3 BacktestEngine 真 5 步（方案 A：engine 层抽 pipeline.py + Scorer.aggregate_pipeline + BacktestDataBundle 扩 active_weights_by_date / industry_by_date / market_cap_by_date）
       ↓
...
| R14-OPEN-3 | §14-3 真 5 步 universe 阈值 | 不硬编码，从 ScoringService 既有 `WINSORIZE_MIN_SAMPLES` 常量 import（与 §14-2 candidate_pool 默认 50 上限同源对齐）|
