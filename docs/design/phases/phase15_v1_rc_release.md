...
> 版本：v1.3（8 子项全交付 + 收尾门槛全过，2026-06-29 RC 验收完成）
> 状态：设计中 → 待评审 → TDD/验收实施
...
| v1.3 | 2026-06-29 | **RC 验收完成，8 子项全交付**。15-1 五链路 5/5（§10.1）/ 15-2 跨制度 30/30（§10.2）/ 15-6 RC tag `v1.0-rc1` + 部署演练数据零变动（§10.3）/ 收尾门槛 ruff+661 UT/E2E+156 INT+vue-tsc 全过、冒烟对生产 105 passed（§10.4）。**RC 验收期发现并根治真问题**：回测 `backtest_max_window_days` 护栏挡不住"短区间也 OOM"——单个 6 日回测拖垮 2GB 生产机（/health 超时 11 分钟）；用户拍板生产直接禁用回测，新增 `backtest_enabled` 开关（False→503 提示本地运行），生产 `.env.prod` + compose 白名单置 false，TDD E2E-BT-09b + 冒烟自适应（§10.4b） |
...
- [x] 收尾门槛：ruff 0 error / unit+e2e **661** / integration **156** / vue-tsc 0 / 冒烟 **105 passed+2 skipped**（见 §10.4）
- [x] 文档头版本与修订历史一致；经验沉淀 CLAUDE.md / memory
...
### 10.4 收尾门槛 + 冒烟验收 + 回测生产禁用补丁（2026-06-29）

**本地收尾门槛全过**：`ruff check src/ tests/` 0 error / `pytest tests/unit tests/e2e` **661 passed** / `pytest tests/integration`（:5433 测试库）**156 passed** / `vue-tsc --noEmit` 0 error。

**冒烟（对生产 RC 实例 quant.portableagi.com）**：**105 passed + 2 skipped**（50s）。修正过程中发现并处置：

**(a) 5 个 stale 冒烟测试对齐当前契约**（均非缺陷，是测试漂移）：
- API-44：POST /positions 已删（§14-10 废手工录入）→ 期望 401 改 **405**
- API-86：`weights_source` 按设计不在 GET /signals 列表响应（仅在 lineage 三层）→ 去掉该断言，保 composite_z/composite_pct_in_market/trigger_reason 3 字段
- API-103：测试 key `smoke-{uuid4}`=42 字符 > 字段 max 36 → 改 `uuid4().hex`（32 字符）
- API-67/71/73：回测窗口由 87 天改 6 天（落护栏内）

**(b) 真问题——回测护栏不足以防 2GB OOM（事故 + 根治）**：
- **事故**：首轮全量冒烟触发 API-67 向生产 `POST /backtest/run`（6 日窗口），单个回测在 2GB 机吃 ~1.5GB → `/health` 超时 11 分钟（级联 21 个超时"失败"）。重启 backend 恢复，孤儿任务自动回收为 FAILED。
- **根因**：`backtest_max_window_days` 护栏只挡长区间，**挡不住"短区间也 OOM"**——2GB 机本质跑不动任何真实回测。
- **根治（用户拍板"生产直接禁用回测"）**：新增 `settings.backtest_enabled`（默认 True）开关，置于 `run_backtest` 最前置；False → **503** 提示走本地算力中心（`scripts/run_backtest_local.py` → `POST /backtest/import` 回灌）。生产 `.env.prod` + root `docker-compose.prod.yml` environment 白名单加 `BACKTEST_ENABLED=false`。
- **TDD**：E2E-BT-09b 禁用 503 用例 RED→GREEN；冒烟 API-67/71/73 改自适应（生产 503→skip / 本地 200→PENDING）。
- **生产实证**：`POST /backtest/run` → 503「回测已在本服务器禁用」；内存稳定 810/1157MB；账户冒烟入金（fund_flow 102，0.01）已 void 还原（cash 15049.07→**15049.06**，回 §10.1 基线）。
- 部署机制坑留档：backend 容器 env 来自 root compose 的**显式 `environment:` 白名单**（非 .env.prod 全量透传）；新增 env 变量必须同时在 compose environment 块加一行 `KEY: ${KEY:-default}`，否则进不了容器。

**结论：收尾门槛 + 冒烟全 PASS；回测生产禁用根治 OOM 宕机风险；生产洁净。**
