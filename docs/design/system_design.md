...
| **Phase 14** | **账户资金链 + 5y candidate_pool 回填 + ICIR 历史回算 + BacktestEngine 真 5 步（V1.0 收尾，~5-8 pd）** | (1) **RM-13 deposit 幂等**（idempotency_key 机制）；(2) **5y candidate_pool 历史回填**——Phase 11 收尾时 `strategy_weights_history` 仍全为 `default_matrix`（手动跑过 `apply_monthly_rebalance(2026-04-30)` 写入 12 行验证 Job 路径通），原因是 ICIR 滚动累积需 ≥ 272 日（`ic_window_days=252 + icir_lag_days=20`）候选池历史，5y × 1210 trade_date 全量 pipeline 回填 + ICIR 历史回算 才能让 `weights_source` 切到 `icir`；(3) **BacktestEngine 真 5 步管线接入**——Phase 11 BacktestEngine 走 `aggregate_legacy` + 后处理派生 composite_z / composite_pct_in_market（避免单股 mock 场景被 5 步管线打死），不影响实盘 critical path，本阶段与 ICIR 历史回填同批改 `engine/backtest/engine.py` 走真 5 步管线；(4) **回测引擎 §2.1 ICIR 校准最小集**（IC 时序量级验证 / 多场景对比 / 滑点敏感性最小验证）；(5) **ICIR 窗口改严格交易日** ✅ **2026-05-26 完成**（Phase 11 实施评审 §6.3 第 8 项）：`services/factor_monitor_service.py::rolling_icir_state` 原用 `timedelta(days=20/272)` 日历日近似（lag≈14 交易日 / warmup≈188 交易日，比 SDD §7.4 "20/272 交易日" 短 ~25%）；本批改为 `FactorMonitorService.__init__(calendar)` 注入 TradingCalendar，`end = calendar.get_prev_trade_date(trade_date, n=20)` + `start = calendar.get_prev_trade_date(end, n=252)` 严格交易日窗口；4 处生产路径（deps.py / monthly_scheduler.py × 2 / daily_pipeline.py）全部注入 `self._calendar`；SDD §7.4 v1.4 措辞早已写"交易日"无需改；UT-P14-5-01 + INT-P14-5-01 跨周末校验全部 PASS；(6) **factor_ic_window_state 共表拆分评估**（Phase 11 实施评审 §6.3 第 9 项）：当前 daily 行（仅 ic_value/sample_size）+ aggregate 行（icir/CI/t_stat/...）共表 + 同 UNIQUE 约束 `(strategy, factor, state, trade_date)`，靠 `ic_value/icir IS NOT NULL` 区分行类型——5y × 250 × 4 × 4 × 3 ≈ 1.2M 行后表膨胀 + 索引不能加速 NOT NULL 谓词；本阶段评估拆为 `factor_ic_daily`（窄表）+ `factor_ic_window_state`（聚合表）或加 `row_type` 列 + partial unique index；(7) **Phase 13 实施评审 P2 6 项**（详见 `docs/reviews/phase13_implementation_review_2026-05-22.md` §8）——R13-P2-1 `DataQualityRepository` 改 instance 方法 + 取消 `repo._session` 私有访问（违反 §6 Service 层规范）/ R13-P2-2 `ingest_daily` 异常分支也写 data_quality_metric 占位行（exception_occurred=1）/ R13-P2-3 `apply_monthly_rebalance` 加优先级控制（命中持续告警时抑制单月告警，避免同月双发）/ R13-P2-4 `API_REQUEST_DURATION` middleware 用 `request.scope["route"].path` template 替代 raw path 防 series 基数爆炸 / R13-P2-5 WS error 帧改 `{code, data, msg}` 统一格式 / R13-P2-6 main.py lifespan finally 加 `await app.state.redis.aclose()`；(8) **Phase 12 实施评审 P1-2 + P2 防御 2 项**（详见 `docs/reviews/phase12_implementation_review_2026-05-20.md` §3 P1-2 / §4 P2-2 + §4 P2-3）——R12-P1-2 `AttributionService.run_monthly` 的 `timedelta(days=int(lookback_months*30.5))` 日历天近似与 ICIR 窗口同源，本批与子项 (5) 严格交易日同批改 `TradingCalendar.get_prev_trade_date(month_end, n=20*lookback_months)` / R12-AttrBackfill `AttributionService` 日级历史回填脚本（与 5y candidate_pool 回填同批跑出 5y × ~60 month_end attribution 全量历史，作为 RC 真机验收预备）+ attribution_repository docstring 顶部加注释（N × 4 ≥ 8000 行时启用 `_BATCH_SIZE=500` 循环防 asyncpg 32767 限制） | phase14_account_integrity.md v1.1 ✅（2026-05-25 评审 P1×3 + P2×4 + P3×6 全收口；R12-P1-2 已在 Phase 13 启动核查交付，本 phase 仅含 R12-AttrBackfill + R12-32767 共 2 项）|
