...
| **Phase 12** | **信号可解释性 / 因子级溯源（V1.0 收尾，~6-10 pd）** | LineageService 重构（返回因子层 raw_factors / 中性化前/后值 / 正交化前/后值）+ SignalCard / SignalLineageView 分层视图（L1 业务可解释 / L2 含 ICIR + 中性化 / L3 含正交化残差 + 审计）+ 多因子回归归因（OLS 收益拆解）；吸收 V1.5-B 全部（S1-GAP-01 + D1-GAP-02）+ V1.5-E 多因子回归 | phase12_factor_lineage.md ✅ |
