...
CREATE TABLE signal (
    ...
    signal_strength VARCHAR(10),                 -- 'STRONG'(top 1%)/'MODERATE'(top 1~5%)；分位制（Phase 11 工业化，SDD §9.1）；仅买入信号使用
...
class SignalGenerator:
    def generate(
        ...
        signal_strength：`composite_pct_in_market ≤ 1%` → 'STRONG'；`1~5%` → 'MODERATE'（分位制，Phase 11 工业化 SDD §9.1；绝对分 ≥90/80-89 仅保留为旧路径 fallback）（仅买入信号使用）
        """
...
- `signal.generate()`: 同一标的不重复信号；买入价区间 [close×0.99, close×1.02]；加仓条件验证（盈利 OR 偏离成本≤±10%）；signal_strength 正确分类（STRONG/MODERATE 按分位 top 1% / top 5%，SDD §9.1）
...
> **2026-05-14 V1.0 定位调整**：2026-05-13 5y 真机验收发现核心评分公式存在根本缺陷（rank-pct 跨期不可比 + 4 策略横截面反相关锁死 + 绝对阈值 80 失效 → 5y 历史信号表 0 行）→ V1.0 重新定位为"**所有阻断用户达成核心目标的问题修复完成才能发布**"。原 Phase 1~10 完成的工作不变，新增 Phase 11~15 作为 V1.0 收尾批次。详见 §9 末注释 + `docs/design/v1_post_release_roadmap.md`（v2.0 重构后承担 V1.0 发布后路线）。
...
| **Phase 13** | **生产可观测 + 部署评审并入（V1.0 收尾，~8-12 pd）** | **完成 ✓ 2026-05-22 + P0/P1 补丁批 2026-05-22 完成**：P13-A MetricsRegistry（7 Counter + 3 Gauge + 2 Histogram）+ SchedulerHealthService + `/metrics` + `/api/v1/health/scheduler` + `/api/v1/health/data` + 4 处埋点接入（pipeline/signal/notification/scheduler）；P13-B alembic 0012 DataQualityMetric 表 + Repository + DataService.ingest_daily 写入 + FactorMonitorService.check_persistent_decay（连续 3 月 ICIR < 0.05 触发 `factor_decayed_persistent`）+ NotificationService.notify_health_alert（HEALTH_ALERT 类型复用 notify_risk_warn 父开关）；P13-C SecretFilter（5 类正则）+ AKShareAdapter 补全 fetch_daily_quotes（按 ts_code 单股 + 1000 上限）/ fetch_index_history（sh000300/sz399001 symbol）+ DataService `_fetch_daily_quotes_with_fallback` 自动降级路径 + DATA_SOURCE_FALLBACK 4 状态 + 双失败 notify_health_alert；P13-D Redis lifespan 实例化 + DailyPipeline._publish_progress 12 个埋点 + WS `/api/v1/pipeline/progress` + frontend `api/websocket.ts` WebSocketClient（auto-reconnect maxRetries=5）+ DashboardView 顶部 PipelineProgressCard；P13-E `docker-compose.monitoring.yml` profile=monitoring + `infra/prometheus/prometheus.yml` + Grafana provisioning（数据源 + Overview 7-panel 仪表盘）+ nginx `/metrics` 内网白名单（allow 127.0.0.1/172.16/10/192.168，deny all）+ deployment.md §13 监控栈章节；P13-F 冒烟 API-96~101（/metrics + /health/scheduler/data + WS 端点已注册）+ Phase 13 收尾。**P0/P1 补丁批 2026-05-22**（评审报告 `docs/reviews/phase13_implementation_review_2026-05-22.md` §8）：R13-P0-1 trigger 注入 redis+notification_channel（手动触发场景下 WS 进度推送恢复）/ R13-P0-2 AKShare fallback 用 PIT 活股 + 1000 上限（Tushare 故障期数据连续性恢复）/ R13-P1-1 PIPELINE_DURATION 6 step + pipeline_total + BACKTEST_QUEUE_DEPTH inc/dec + DATA_LATENCY 双路径 set / R13-P1-2 apply_monthly_rebalance 接入 check_persistent_decay + MonthlyScheduler 注入 notifier（持续告警生产路径连通）/ R13-P1-3 pipeline 失败改 notify_health_alert("pipeline_failed")（HEALTH_ALERT 统一入口）/ R13-P1-4 vite ws:true + frontend/nginx WS upgrade location（开发环境 WS 可用）；P2 6 项归 Phase 14 启动核查、P3 5 项归 V1.5-A。测试：UT 28 新增（设计期）+ 11 新增（补丁批）+ E2E-P13-A/B/D 6 + 1 trigger_redis_injection + INT-P13-A-01/02 2 + 冒烟 6 全 PASS；回归 unit+e2e 552 + integration 118 + ruff 0 error + vue-tsc 0 error。原"修复预存在冒烟 API-67/73/83 3 项"已在 Phase 12 收尾期处置。OpenTelemetry trace / AlertManager / AKShare 财务+分红+namechange / APScheduler 集群化推迟 V1.5+。吸收 V1.5-H 全部 5 P2（S5-GAP-01/02/03 + S2-GAP-01 + D4-GAP-03）+ Phase 10 评审 G-1（WS 前端消费）+ G-2（AKShare 自动降级）| phase13_production_observability.md v1.1 ✅ + 实施评审 v1.0 ✅ |
| **Phase 14** | **账户资金链 + 5y candidate_pool 回填 + ICIR 历史回算 + BacktestEngine 真 5 步 + 日级 IC 生产者（V1.0 收尾，~7-12 pd）** | (1) **RM-13 deposit 幂等**（idempotency_key 机制）；(2) **5y candidate_pool 历史回填**——Phase 11 收尾时 `strategy_weights_history` 仍全为 `default_matrix`（手动跑过 `apply_monthly_rebalance(2026-04-30)` 写入 12 行验证 Job 路径通），原因是 ICIR 滚动累积需 ≥ 272 日（`ic_window_days=252 + icir_lag_days=20`）候选池历史，5y × 1210 trade_date 全量 pipeline 回填 + ICIR 历史回算 才能让 `weights_source` 切到 `icir`（**2026-06-02 迁移准备期发现 item (2) 隐藏未交付：ICIR 历史回算实跑只产 default_matrix，根因是日级 IC 时序无任何生产者，详见 item (9) + §14-9**）；(3) **BacktestEngine 真 5 步管线接入**——Phase 11 BacktestEngine 走 `aggregate_legacy` + 后处理派生 composite_z / composite_pct_in_market（避免单股 mock 场景被 5 步管线打死），不影响实盘 critical path，本阶段与 ICIR 历史回填同批改 `engine/backtest/engine.py` 走真 5 步管线；(4) **回测引擎 §2.1 ICIR 校准最小集**（IC 时序量级验证 / 多场景对比 / 滑点敏感性最小验证）；(5) **ICIR 窗口改严格交易日** ✅ **2026-05-26 完成**（Phase 11 实施评审 §6.3 第 8 项）：`services/factor_monitor_service.py::rolling_icir_state` 原用 `timedelta(days=20/272)` 日历日近似（lag≈14 交易日 / warmup≈188 交易日，比 SDD §7.4 "20/272 交易日" 短 ~25%）；本批改为 `FactorMonitorService.__init__(calendar)` 注入 TradingCalendar，`end = calendar.get_prev_trade_date(trade_date, n=20)` + `start = calendar.get_prev_trade_date(end, n=252)` 严格交易日窗口；4 处生产路径（deps.py / monthly_scheduler.py × 2 / daily_pipeline.py）全部注入 `self._calendar`；SDD §7.4 v1.4 措辞早已写"交易日"无需改；UT-P14-5-01 + INT-P14-5-01 跨周末校验全部 PASS；(6) **factor_ic_window_state 共表拆分评估** ✅ **2026-05-26 完成**（Phase 11 实施评审 §6.3 第 9 项）：方案 A 落地——alembic 0014 加 `row_type VARCHAR(16) NOT NULL DEFAULT 'daily'` 列 + 回填存量（`icir IS NOT NULL` → 'aggregate'）+ partial unique index `(strategy, factor, state, trade_date) WHERE row_type='aggregate'`；ORM `FactorICWindowState.row_type` 字段；Repository `upsert_ic_daily` 写 'daily' / `upsert_ic_aggregate` 写 'aggregate'（on_conflict 升级 row_type）/ `get_recent_aggregates / list_aggregates / get_latest_icir` 过滤改 `row_type='aggregate'` 替代 `icir IS NOT NULL` 谓词走 index-only scan；6 INT-P14-6 集成测试覆盖 daily/aggregate 写入 + 同 4-tuple 升级 + partial unique 行为 + 查询过滤；方案 B（拆 factor_ic_daily 窄表 + factor_ic_window_state 聚合表）留 V1.5+ DBA 视图；(7) **Phase 13 实施评审 P2 6 项** ✅ **2026-05-26 完成**（详见 `docs/reviews/phase13_implementation_review_2026-05-22.md` §8.1.1）——R13-P2-1 `_record_validation` + `fetch_dividends` 用 `repo.session` 公共属性 + `AccountService.get_all_positions()` 替代 `repo._session` 私有访问 / R13-P2-2 `ingest_daily` 加 finally 写 `metric_key='exception_occurred'` 占位（异常→1 / 正常→0）/ R13-P2-3 `apply_monthly_rebalance` 累积 `persistent_decay_hits` set + R1/R2/R3 action 单月告警通过该 set 抑制同月重复 / R13-P2-4 `API_REQUEST_DURATION` middleware 用 `request.scope["route"].path` template 替代 raw path / R13-P2-5 WS error 帧改 `{code, data, msg}` + 前端 PipelineProgressCard 适配 / R13-P2-6 main.py lifespan finally 加 `await app.state.redis.aclose()` best-effort；(8) **Phase 12 实施评审 P1-2 + P2 防御 2 项** ✅ **2026-05-26 完成**（详见 `docs/reviews/phase12_implementation_review_2026-05-20.md` §3 P1-2 / §4 P2-3）——R12-P1-2 严格交易日已在 Phase 13 启动核查交付 / R12-AttrBackfill 新增 `scripts/backfill_attribution_history.py`（双模式：默认跳过已有月份 + `--force` 强制重写 + `--dry-run-plan` 预检；SIGINT/SIGTERM graceful；月末严格交易日定位 = `calendar.offset_trade_date(month_last_calendar, 0)` 自动跳周末/节假日）+ INT-P14-8-01/02 集成测试覆盖 6 month_end × 4 factor = 24 行写入 + `_enumerate_month_ends` 跨周末校验 / R12-P2-3 `attribution_repository.py` docstring 顶部加 V1.0 ≤ 1200 行未达 32767 限制 + N × 4 ≥ 4000 行应启用 `_BATCH_SIZE=500` 防御注释；(9) **日级 IC 生产者（§14-9，ICIR 历史回算补全）** ✅ **完成**（5y 日级 IC 回填 4748 行 bad=0 + 重跑月末批 `--force` 切 `icir`，OSCILLATION 175 样本真 icir 证明端到端通；真机验收 PASS，详见 memory phase14_9_icir_acceptance）——迁移准备期发现 item (2) 的 ICIR 历史回算只产 `default_matrix`，根因是日级 IC 时序 `IC_daily(s,f,t)=corr(factor_value_{t-20}, return_{t-20→t})`（SDD §7.4，全 universe Spearman Rank IC）**无任何生产者**（`upsert_ic_daily` 仅测试调用，日常 pipeline CP2 只读权重不写日级 IC）→ `rolling_icir_state` 各 state 子集恒 < 60 样本回退冷启动；§14-9 落地：engine 纯函数（复用 `FactorMonitorEngine.calc_ic`）+ `scripts/backfill_daily_ic.py`（5y 全 universe 逐日重评 strategy_z × 前向收益 → 写 `row_type='daily'`）+ 实盘 CP2 续算接线（20 日 lag）+ 重跑 §14-2 月末批 `--force` 切 `icir`；解锁 item (4) ICIR 校准最小集；(10) **成交/资金流水作废订正（§14-10）** ✅ **2026-06-18 完成**（commit b700c86，alembic 0016）——前台持仓/交易明细/资金流水原 append-only、录错无订正经路（且 WAC 增量上写 SELL 不还原致冲正也无法还原误买、前端「交易明细」误按不存在 `flow_type='TRADE'` 过滤致成交不展示）；B 方案落地：`is_voided/voided_at/void_note` 三列 + 纯函数 `replay_position`（持仓=非作废成交+分红派生视图）+ `void_trade`（dry-run 校验超卖→联动作废费用流水+现金逆仕訳+replay 重建）+ `void_fund_flow`（费用流水禁单独作废）+ `GET /account/trades` 修复成交明细 + void 端点 + 前端撤销/订正/voided 灰显；UT replay 10 例 + INT-VOID-01~07 + E2E aapi-19~26 + 冒烟 API-104~107；**附带数据修复**：每日管线 `ingest_daily` 不传 `_st_codes` → is_st 全 False（05-20 起）→ *ST 仙股混入买入信号，commit 421c56d `DataService._build_current_st_codes` 自愈 + `backfill_is_st.py` 回填，06-18 自愈验证买入信号 *ST=0；**一致性收口（2026-06-24）**：持仓既定义为成交流水派生视图，遂废除 `POST /positions`/`add_position`/前端手工录入持仓（手工持仓无成交支撑、不扣现金，void/replay 必丢弃致账务不整合），建仓改走 `POST /account/trades` 开仓 BUY | phase14_account_integrity.md v1.4 ✅（v1.1~v1.3-r1 评审全收口；§14-9 5y 回填+ICIR rebalance 真机验收 PASS；v1.4 新增 §14-10 作废订正）|
| **Phase 15** | **V1.0 RC 验收 + 文档校核（V1.0 收尾，~7-10 pd）** | (1) **5y 真机端到端验收**（评分链路 candidate top ≥ 85 / 信号链路 BUY（原始 pipeline ~40~50/日，与 phase11 §10.4 实测对账；用户可执行 5~15/日＝经 RiskChecker/持仓/watchlist 二次过滤后，SDD §9.1）/ 因子溯源完整 / 监控链路告警 PASS / 账户幂等 PASS）；(2) **跨制度回归（phase11 §10.4） 3 state × 10 trade_date = 30 日完整版**（Phase 11 收尾时只跑了 4 trade_date 抽样，理由是生产 5y 数据上每日 pipeline 130~1600s、30 日全跑 ~10 小时不适合 Phase 11 单次回归门槛——本阶段与 Phase 14 § 5y 全量 pipeline 回填同批一并跑出 30 日完整版）；(3) **STRONG 基线相对百分比化**（Phase 11 v1.4 5y 实测 STRONG count 18~23 vs 设计 §10.4 "≥ 30 只"——当前 V1.0 universe 过滤后 ~2400 只，top 1% ≈ 24，设计绝对数 30 假设全市场 3200，本阶段把 §10.4 基线改为相对百分比并校核 SDD §7-10 对应条款）；(4) **覆盖率门槛验证**（Phase 11 留的 Engine 层 ≥ 90% 覆盖率统计）；(5) **文档校核**（SDD §7-10 已在 v1.4 合入，本阶段仅做交叉一致性核对 + 残留 P2 收尾，例如 SDD §7.2.1 共线性附注 / CLAUDE.md Phase 9 行状态等遗留小项 + `v1_5_roadmap.md` 重命名 `v1_post_release_roadmap.md`）；(6) V1.0 RC 标签 + 生产 Docker 部署演练；(7) **`factor_ic_history` 旧表归并 + DROP**（report_service / monthly_scheduler 读写迁移到 `factor_ic_window_state` + alembic DROP 旧表 + 生产数据迁移；消费 phase11 评审"Phase 12 或 Phase 15 末归并旧表并 DROP"，Phase 12 未做、本阶段在 V1.0 内清掉僵尸双表）；(8) **Phase 12 评审 P1 合规 DisclaimerBanner**（归因视图补合规声明，与 BacktestLimitationsBanner / 三视图 DisclaimerBanner 视觉一致；RC 前必修，否则外部合规复审退） | phase15_v1_rc_release.md（待创建）|
...
> - **Phase 11~15 估算合计 ~40-62 pd（8-12 周）**（2026-06-26 Phase 15 启动核查 6-8→7-10 pd 同步；区间为各 phase 分项精确和 12-18+6-10+8-12+7-12+7-10）：Phase 11 ~12-18 pd（评分管线重构 + ICIR 工程化主体）+ Phase 12 ~6-10 pd（前端 L1/L2/L3 视图 + 多因子回归归因）+ Phase 13 ~8-12 pd（Prometheus/OTel/告警栈）+ Phase 14 ~7-12 pd（RM-13 + 5y 回填 + ICIR 历史回算 + BacktestEngine 真 5 步 + ICIR 校准最小集 + Phase 13 P2 6 项 + Phase 12 P2 2 项 + §14-9 日级 IC 生产者；Phase 14 行同步自 v1.1 评审 P2-3 2026-05-25 + v1.3 §14-9 新增 2026-06-02）+ Phase 15 ~7-10 pd（5y 真机 RC 验收 + 30 日跨制度回归 + 文档校核 + factor_ic_history 旧表 DROP + Phase 12 P1 合规 banner + 生产部署演练；2026-06-26 启动核查 +1~2 pd）。pd 拆分依据见 `docs/design/v1_post_release_roadmap.md` v2.0 §★ 升级清单 + Phase 11 设计文档 §14（已交付 v1.1，2026-05-15）+ Phase 12~15 设计文档（待创建）。
...
> - **V1.0 发布后 V1.5+ 完整 scope（SDD §16 11 项产品功能 + V1.0 评审 P2/P3 15 项剩余 + SDD 外部专家评审 7 项剩余 + Phase 10 评审 1 项剩余 = 34 项；含已升级 V1.0 清单、推迟原因、修复条件、估算）见 `docs/design/v1_post_release_roadmap.md`（v2.0 重构后）**——所有推迟项不在 V1.0 Phase 范围内，V1.5 启动（V1.0 RC 后）按该路线图 §6 主题（V1.5-A..J）打包。
