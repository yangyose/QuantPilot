...
| **Phase 13** | **生产可观测 + 部署评审并入（V1.0 收尾，~8-12 pd）** | Prometheus 指标埋点（7 Counter + 3 Gauge + 2 Histogram）+ 调度器健康端点（/health/scheduler + /health/data）+ 日志 SecretFilter（5 类正则：TUSHARE_TOKEN / bcrypt / Bearer / WxPusher app/uid / REDIS_URL）+ DataValidator 错误持久化（DataQualityMetric 表 + alembic 0012）+ 因子衰减监控（FactorMonitorService 持续告警 N=3 月，ICIR < 0.05 触发 notify_factor_alert("factor_decayed_persistent")）+ Pipeline / DB / 数据延迟告警通过 NotificationService.notify_health_alert + Grafana 监控 stack（docker-compose.monitoring.yml profile=monitoring）+ AKShare 自动降级（fetch_daily_quotes + fetch_index_history 补全 + DataService Tushare→AKShare fallback 路径）+ WebSocket 前端消费（/pipeline/progress 后端 WS 新增 + frontend WebSocketClient + PipelineView/BacktestRunView 接入）+ /factor-quality/icir-timeseries 端点；**Phase 13 启动核查清单含修复 Phase 12 收尾发现的预存在冒烟 3 项失败（API-67 / API-73 / API-83）**——P13-F1 启动前必修，不延后到 Phase 14。OpenTelemetry trace / AlertManager / AKShare 财务+分红+namechange / APScheduler 集群化推迟 V1.5+。设计文档 v1.0 已交付 2026-05-21；吸收 V1.5-H 全部 5 P2（S5-GAP-01/02/03 + S2-GAP-01 + D4-GAP-03）+ Phase 10 评审 G-1（WS 前端消费）+ G-2（AKShare 自动降级）| phase13_production_observability.md v1.0 ✅ |
