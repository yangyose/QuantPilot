...
> - **Phase 11~15 估算合计 ~40-59 pd（8-12 周）**：Phase 11 ~12-18 pd（评分管线重构 + ICIR 工程化主体）+ Phase 12 ~6-10 pd（前端 L1/L2/L3 视图 + 多因子回归归因）+ Phase 13 ~8-12 pd（Prometheus/OTel/告警栈）+ Phase 14 ~7-12 pd（RM-13 + 5y 回填 + ICIR 历史回算 + BacktestEngine 真 5 步 + ICIR 校准最小集 + Phase 13 P2 6 项 + Phase 12 P2 2 项 + §14-9 日级 IC 生产者；Phase 14 行同步自 v1.1 评审 P2-3 2026-05-25 + v1.3 §14-9 新增 2026-06-02）+ Phase 15 ~6-8 pd（5y 真机 RC 验收 + 文档校核 + 生产部署演练）。pd 拆分依据见 `docs/design/v1_5_roadmap.md` v2.0 §★ 升级清单 + Phase 11 设计文档 §14（已交付 v1.1，2026-05-15）+ Phase 12~15 设计文档（待创建）。
