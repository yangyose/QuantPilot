...
from quantpilot.api.deps import get_current_account_id, get_report_service
...
async def get_report(
    ...
    """GET /reports/{report_id} → 报告详情（含完整 content JSON）。

    不存在或不属当前用户账户 → 404（ownership，不泄露存在性）。
    """
    report = await service.get_by_id(report_id, account_id)
    if report is None:
        raise HTTPException(
...
async def generate_report(
    ...
    """POST /reports/generate → 为当前用户账户生成自定义时间段报告。返回 ReportItem。"""
    report = await service.generate_custom(body.start_date, body.end_date, account_id)
    return {
        "code": 0,
