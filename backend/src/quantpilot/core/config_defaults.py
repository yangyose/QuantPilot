...
class ValueStrategyConfig:
    ...
    # ── `roe_quality` 是否计入合成 ────────────────────────────────────────────
    # **默认 True。2026-09-09 专项实测后的结论是「维持现状」——证据不支持剔除。**
    # 开关保留下来只是为了将来能一键关掉重测，不是「打算关但还没关」。
    #
    # 起初的怀疑是有根据的：前视污染修复前该因子 IC +0.0183、修复后 −0.0028（变号；
    ...
    #
    # 但 A/B/C 三口径实测（35 日，开发集，h=20）推翻了「该剔」这个结论：
    #   A 现行 pe+pb+roe    IC +0.0319  低 ROE 组平均分位 **0.351**
    #   B 剔除 pe+pb        IC +0.0398  低 ROE 组平均分位  0.407
    #   C 换 F-Score        IC +0.0380  低 ROE 组平均分位  0.406
    # ① alpha 上三者**无法区分**（配对差值 |t| ≤ 1.47，全不显著）；
    # ② 陷阱区分能力上 **A 明显最好**，而 **F-Score 顶替不了 ROE**（C ≈ B）。
    #
    # ⚠️ **本条最值得记的**：`roe_quality` 同时承担**两个职责**——一个（弱）alpha
    # 因子，和价值陷阱的**区分维度**。此前只测 alpha 就得出「该剔」（ΔIC +0.0062、
    # t=1.48），是把第二个职责悄悄扔了。**一个因子可能身兼数职，只测其中一个
    # 就下剔除结论必然出错。**
    #
    # 另两点澄清：**不是「ROE 数据无效」**（覆盖率 98.8%、构造正确、污染已修干净）；
    # 真要剔除还需先按 C-5 回写 SDD §7.2.4（该表把「ROE 质量」列为 35% 权重因子，
    # 并把陷阱规避写成「需结合 ROE 质量因子过滤」）。
    # 全过程见 docs/reviews/scoring_monotonicity_2026-09-09.md §8~§10。
